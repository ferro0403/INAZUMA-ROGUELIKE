...
(async () => {
  ...
  assert.deepStrictEqual(await core.validateHallDocument('hall_1', hallDoc, validManifest, crypto), prepared.hallEntries[0]);
  const encodedHallDoc = { ...hallDoc, payloadEncoding: core.PAYLOAD_ENCODING, payload: core.encodeFirestorePayload(hallDoc.payload) };
  assert.deepStrictEqual(await core.validateHallDocument('hall_1', encodedHallDoc, validManifest, crypto), prepared.hallEntries[0]);
  ...
  const identityFreePayload = { teamName: 'Legacy Raimon', fullRoster: [] }, identityFreeHash = await core.hash(identityFreePayload, crypto);
  const identityFreeEntry = await core.validateHallDocument('legacy-hall', { schemaVersion: 1, revision: 1, hallTeamId: 'legacy-hall', archiveKey: 'legacy::archive', payload: identityFreePayload, payloadHash: identityFreeHash }, { ...validManifest, hallTeamIds: ['legacy-hall'], hallTeamHashes: { 'legacy-hall': identityFreeHash } }, crypto);
  const identityFreeSnapshot = core.reconstructSnapshot({ ...prepared.payloads, hall_index: { ...prepared.payloads.hall_index, teamIds: ['legacy-hall'], count: 1 } }, [identityFreeEntry]);
  const identityFreePrepared = await core.prepareSnapshot(identityFreeSnapshot, crypto);
  assert.deepStrictEqual(identityFreePrepared.hallEntries[0], { hallTeamId: 'legacy-hall', archiveKey: 'legacy::archive', payload: identityFreePayload, payloadHash: identityFreeHash });
  assert.strictEqual(Object.values(identityFreePrepared.hallEntries[0]).includes(undefined), false);
