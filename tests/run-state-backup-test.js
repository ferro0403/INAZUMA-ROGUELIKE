...
assert.equal(recovered.lives, 2, "backup recovered silently and keeps the global life cap");
assert.equal(JSON.parse(env.store.get(keys.primary)).lives, 2, "backup restored as primary with the global life cap");
env.store.set(keys.primary, "{broken");
