...
const PRE_APP_PRODUCTION_MODULES = [
  ...
  "storage/album-permanent-effects.js",
];
...
async function draftFromCreation(runtime, seasonDb, meta, random, coverage) {
  ...
  // The real browser IndexedDB cutover drains Album receipts asynchronously
  // after the draft commit. Wait for that background drain before the very
  // next gameplay action: the pre-fix race advanced canonical generation here
  // while leaving app.js on the previous generation.
  await settle();
  await settle();
  const canonicalAfterAlbumDrain = runtime.canonical;
  const liveAfterAlbumDrain = runtime.seam.getRun();
  const albumReceipts = (canonicalAfterAlbumDrain.permanentEffectOutbox || []).filter((effect) => effect.type === "album-unlock");
  expect(albumReceipts.length >= runtime.canonical.roster.length, "IndexedDB Album soak produced no draft receipts", meta, canonicalAfterAlbumDrain, { albumReceipts: albumReceipts.length });
  expect(albumReceipts.every((effect) => effect.status === "applied"), "IndexedDB Album background drain left draft receipts pending", meta, canonicalAfterAlbumDrain, { pending: albumReceipts.filter((effect) => effect.status !== "applied").map((effect) => effect.id) });
  expect(Number(liveAfterAlbumDrain?.storageGeneration || 0) === Number(canonicalAfterAlbumDrain.storageGeneration || 0), "IndexedDB Album drain desynchronized live/canonical generation", meta, canonicalAfterAlbumDrain, { liveGeneration: liveAfterAlbumDrain?.storageGeneration, canonicalGeneration: canonicalAfterAlbumDrain.storageGeneration });
  coverage.albumDrainChecks += 1;
