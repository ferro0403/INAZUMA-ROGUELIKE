...
function makeContext(testMode=true) {
  ...
  const c={console,structuredClone,URLSearchParams,location:{search:""},document,fetch:()=>new Promise(()=>{}),setTimeout:(fn)=>0,requestAnimationFrame:(fn)=>fn(),globalThis:null,__INAZUMA_TEST_MODE__:testMode,SEASON1_CONFIG:{maxRoster:3,maxInventory:2,itemPool:[],nodeLabels:{pull_free_agents:{label:"Pull"}},categoryRanks:{Normale:1},legendaryCategories:[]},
    ...
    GameplayPersistence:null,RecruitmentPoolRuntime:{choiceDatabase:(src,season,free)=>src==="legacy"?season:free,canonicalPlayerId:p=>String(p.playerId),candidateKey:p=>String(p.profileId||p.playerId),eligible:()=>true,isSeasonProfileCandidate:p=>Boolean(p.profileId)},DraftEngine:{randomFromSeed:()=>()=>0.1,selectCandidates:(a)=>a.slice(0,3),selectWeightedCandidates:(a)=>a.slice(0,3),shuffle:a=>a.slice()},MapEngine:{ensureCurrentZone:()=>({generated:false}),reachableNodeIds:()=>[],completeNode(zone,id){const n=zone.nodes.find(x=>x.id===id);n.completed=true;(zone.completedNodeIds||=[]);if(!zone.completedNodeIds.includes(id))zone.completedNodeIds.push(id);zone.currentNodeId=id;zone.pendingNodeId=null;(zone.path||=[]).push(id);}},RoguelikeRules:{isProfileAwareRosterEntry:()=>false,applyEquipment:s=>s,removeUnavailable:()=>{},resolveDevelopmentEffectiveMetadata:p=>p,unlockedPullLevel:()=>2},InazumaProgression:{getPlayerAtLevel:p=>({...p})},DevelopmentRuntime:{resolvePlayer:(_r,p)=>({...p}),resolveRosterPlayer:(_r,p)=>({...p}),resolveEffectiveMetadata:(_r,p)=>({...p}),rosterEntryPermanentFields:()=>({}),trainingState:(_r,_p,e)=>({applications:e.potentialBoostApplications||[],currentLocalBoost:e.potentialBoost||0,currentOverallBoost:e.currentOverallBoost||0,maxLocalBoost:99,remainingBoost:99-(e.potentialBoost||0)}),planIntensiveTraining:()=>({codexDeltas:{}})},LevelProgression:{formatLevel:e=>String(e.level)},FiveVFive:{removeUnavailable(r){const ids=new Set(r.roster.map(x=>x.playerId));for(const k in r.fiveVFive.lineup)if(!ids.has(r.fiveVFive.lineup[k]))r.fiveVFive.lineup[k]=null;},formationById:()=>null,assign:()=>{},ensure:()=>{},validate:()=>({})},SmartLineup:{optimizeLineupsForNewPlayer:()=>({elevenChanged:false,fiveChanged:false})},RunStatistics:{ACTIONS:{PLAYER_RECRUITED:"PLAYER_RECRUITED",PULL_OPENED:"PULL_OPENED",NODE_COMPLETED:"NODE_COMPLETED"},recordRunAction(r,type,o){r.statistics.actions.push({type,actionId:o.actionId});}},AlbumProgress:{unlockPlayer(id){album.push(id);}},PersistenceBootstrapGate:{ready:new Promise(()=>{})},PermanentEffects:{enqueueAlbum(r,e){(r.permanentEffectOutbox||=[]).push(e);},drain(){return{};}},DevelopmentV2:{DEVELOPMENT_RESOURCE_ASSETS:{coins:"",shards:""},optionsFromUpgrade:()=>({})}};
  c.globalThis=c; c.window=c; c.history={}; c.matchMedia=()=>({matches:false}); vm.createContext(c); for(const file of ["js/recruitment/player-identity.js","js/recruitment/roster-invariants.js","js/recruitment/recruitment-view.js","js/recruitment/recruitment-controller.js","js/pulls/pull-invariants.js","js/gameplay-persistence.js","js/pulls/pull-pool.js","js/pulls/pull-items.js","js/pulls/pull-view.js","js/pulls/pull-controller.js","js/pulls/pull-candidates.js","js/squad/squad-controller.js","js/squad/squad-view.js","js/five-v-five/five-v-five-controller.js","js/five-v-five/five-v-five-view.js","js/five-v-five/five-match-presentation.js","js/special-match/special-match-view.js","js/special-match/special-match-controller.js","js/special-match/special-match-reward-view.js","js/special-match/special-match-reward-controller.js","js/boss/boss-flow-controller.js","js/match/match-presentation.js", "js/match/match-controller.js","js/gameover/gameover-view.js","js/gameover/gameover-controller.js","js/finalization/finalization-view.js","js/finalization/finalization-controller.js","js/inventory/inventory-model.js","js/inventory/item-presenter.js","js/inventory/inventory-controller.js","js/map/node-router.js","js/map/trade-node-controller.js","js/map/run-map-controller.js","js/album/album-view.js","js/album/album-controller.js","js/hall/champion-snapshot.js","js/hall/champion-presentation.js","js/hall/hall-view.js","js/hall/hall-controller.js","js/development/development-center-view.js","js/development/development-center-controller.js","js/home/home-view.js","js/home/home-controller.js","js/shop/shop-view.js","js/shop/shop-controller.js","js/settings/settings-view.js","js/settings/settings-controller.js","js/run-entry/season-selection-view.js","js/run-entry/season-selection-controller.js","js/run-entry/run-resume-controller.js","js/run-entry/initial-draft-view.js","js/run-entry/initial-draft-controller.js","js/app/ui-shell.js", "js/app/app-bootstrap.js", "js/profile/team-profile-runtime.js", "js/app/dev-diagnostics.js", "js/app/test-seams.js", "js/run/run-roster-runtime.js", "js/player/player-visuals.js","js/player/player-view.js","js/player/player-detail-controller.js","js/app.js"])vm.runInContext(fs.readFileSync(file,"utf8"),c);
...
// A normal pull with roster space commits recruitment and node completion together.
{
  const h=makeContext(); vm.runInContext(fs.readFileSync("js/boss-gameover-runtime.js","utf8"),h.c); h.c.SEASON1_CONFIG.maxRoster=4; h.db.bossOrder=[{teamId:"boss",teamName:"Boss"}]; const node={id:"pull-space",type:"pull_free_agents",layer:1,pullState:{pullType:"pull_free_agents",rerolls:0,excludedCandidateIds:[],luckyCharmUsed:false,candidateIds:[]},completed:false}; Object.assign(h.run.currentZone,{nodes:[node],completedNodeIds:[],currentNodeId:"start",pendingNodeId:"pull-space",path:["start"],edges:[]});
  h.c.__INAZUMA_RECRUITMENT_TEST__.openPull(node,"pull_free_agents",{}); const chosen=h.db.players.find(player=>player.playerId==="new"); const completeNode=current=>h.c.__INAZUMA_TERMINAL_FLOW_TEST__.completePullNodeMutation(current,"pull-space","pull_free_agents","new"); h.setFail("QuotaExceededError"); h.c.__INAZUMA_RECRUITMENT_TEST__.recruitPlayer(chosen,"free_agents",2,()=>{}, {transactionMutate:completeNode,onRecover:()=>{}});
  let loaded=h.c.RunState.load(); assert(!loaded.roster.some(x=>x.playerId==="new")); assert.strictEqual(loaded.currentZone.pendingNodeId,"pull-space"); assert.strictEqual(loaded.currentZone.nodes[0].completed,false); assert.strictEqual(loaded.statistics.actions.filter(a=>a.type==="NODE_COMPLETED").length,0);
  h.c.__INAZUMA_RECRUITMENT_TEST__.recruitPlayer(chosen,"free_agents",2,()=>{}, {transactionMutate:completeNode,onRecover:()=>{}}); loaded=h.c.RunState.load(); assert.strictEqual(loaded.roster.filter(x=>x.playerId==="new").length,1); assert.strictEqual(loaded.currentZone.pendingNodeId,null); assert.strictEqual(loaded.currentZone.nodes[0].completed,true); assert.strictEqual(loaded.statistics.actions.filter(a=>a.type==="NODE_COMPLETED").length,1); assert.strictEqual(loaded.phase,"map");
}
console.log("recruitment production path roster-space pull atomicity passed");
