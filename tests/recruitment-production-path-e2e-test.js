...
{
  const h=makeContext(); vm.runInContext(fs.readFileSync("js/boss-gameover-runtime.js","utf8"),h.c); const node={id:"pull-1",type:"pull_free_agents",pullState:{pullType:"pull_free_agents",rerolls:0,excludedCandidateIds:[],luckyCharmUsed:false,candidateIds:[]},completed:false}; h.run.currentZone.nodes=[node];
  ...
  assert(!h.c.RunState.load().roster.some(x=>x.playerId==="new")); assert.strictEqual(node.completed,false); assert(h.modal.children.some(x=>x.dataset.playerId==="new"),"openPull rebuilt valid candidates");
  confirmCurrentOffer(h,"new"); try { h.modal.querySelectorAll(".bench-replacement-grid [data-player-id]").find(x=>x.dataset.playerId==="out").click(); } catch (error) { assert.match(error.message, /normalizeTeamIdentity/, "minimal DOM stops only after committed map navigation"); }
  const loaded=h.c.RunState.load(); assert(loaded.roster.some(x=>x.playerId==="new")); assert(!loaded.roster.some(x=>x.playerId==="out")); assert.strictEqual(loaded.statistics.actions.filter(a=>a.type==="PLAYER_RECRUITED").length,1); assert.strictEqual(loaded.permanentEffectOutbox.length,1);
}
