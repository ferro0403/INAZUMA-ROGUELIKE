...
assert.doesNotMatch(app.match(/const equipmentMarkup[^;]+;/)?.[0] || "", /itemImageFallbackSvg/, "empty equipment does not render a fake image placeholder");
assert.match(app, /player-detail-hero--long-name/, "long names receive an adaptive hero treatment");
...
assert.match(css, /grid-template-columns: 47% 53%/, "hero reserves independent content and fullbody zones");
