...
assert(appJs.includes("const remaining = Math.max(0, Number(pending.rewardsRemaining") && appJs.includes("function syncPendingBossReward(flow)") && appJs.includes("run.pendingBossVictory.rewardsRemaining = flow.remainingRewards"), "boss rewards must be resumed from and persisted to run state");
