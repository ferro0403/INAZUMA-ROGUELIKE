...
for (const name of ['run-state','album-progress','development-v2','hall-of-fame']) assert.match(html, new RegExp(`js/${name}\\.js\\?v=20260801-cloud-sync-3`));
for (const name of ['account-ui','firebase-account','cloud-save-core','firebase-cloud-save']) assert.match(html, new RegExp(`js/${name}\\.js\\?v=20260801-firestore-safe-1`));
...
assert.match(rules, /fresh\.createdAt == old\.createdAt/); assert.match(rules, /fresh\.source == old\.source/); assert.match(rules, /fresh\.schemaVersion == old\.schemaVersion/); assert.match(rules, /fresh\.updatedAt == request\.time/);
