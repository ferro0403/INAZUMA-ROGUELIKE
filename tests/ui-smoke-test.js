...
assert(css.includes(".pull-offer-grid, .mobile-compact-player-list { gap: 10px; }"), "pull and bench replacement cards must share the compact mobile spacing");
assert(/@media \(max-width: 780px\)[\s\S]*?\.mobile-compact-player-list \.player-card-large[\s\S]*?grid-template-columns:\s*82px minmax\(0, 1fr\) 42px/.test(css), "mobile full-roster replacement cards must use the same horizontal compact grid as pull candidates");
assert(/@media \(max-width: 780px\)[\s\S]*?\.pull-offer-grid \.player-card-large \.player-portrait, \.mobile-compact-player-list \.player-card-large \.player-portrait[\s\S]*?height:\s*100%[\s\S]*?object-fit:\s*contain/.test(css), "mobile compact portrait rules must keep visible non-deformed pull portraits");
...
assert(css.includes(".pull-choice-actions") && css.includes(".pull-choice-option.is-selected"), "inline pull confirmations must be styled as selected-card wrappers");
assert(appJs.includes("aria-expanded=\"false\"") && appJs.includes("aria-controls=\"${panelId}\""), "pull card triggers must expose inline confirmation accessibility state");
assert(appJs.includes("showPlayerDetailsFor(player") && appJs.includes("onClose: () =>") && appJs.includes("updateInlinePullSelection(restoredGrid"), "Scheda must use the standard Player Detail and restore the inline confirmation");
