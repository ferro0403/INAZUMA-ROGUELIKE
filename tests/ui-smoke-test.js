...
assert(/@media \(max-width: 780px\)[\s\S]*?\.pitch-row\s*\{[^}]*--pitch-card-size:[\s\S]*?grid-template-columns:\s*repeat\(var\(--players-in-row, 1\), minmax\(0, var\(--pitch-card-size\)\)\)/.test(css), "mobile pitch rows must keep a constant card width while changing only player distribution");
...
assert(css.includes(".candidate-grid .player-card { display: block;"), "mobile candidate cards keep the base player-card reset before contextual overrides");
assert(css.includes(".candidate-grid .player-card-large { display: grid; grid-template-columns: 72px minmax(0, 1fr) 44px;"), "mobile pull cards must use compact horizontal list layout");
...
assert(css.includes('.pitch-row[data-row-count="4"]'), "tactical rows must expose row-count sizing rules for four-player lines");
...
server.listen(0, "127.0.0.1", async () => {
  ...
  try {
    ...
    assert([...window.document.querySelectorAll(".pitch-row")].every((row) => row.dataset.rowCount === String(row.children.length || 1)), "tactical rows expose a data-row-count hook for adaptive sizing");
    assert(css.includes("var(--pitch-card-size)"), "card width must stay constant for one- and two-player rows");
    assert(/@media \(max-width: 780px\)[\s\S]*?\.pitch-row\s*\{[\s\S]*?gap:\s*var\(--pitch-row-gap\)[\s\S]*?margin:\s*12px 0 26px/.test(css), "mobile squad rows must be slightly more spaced without expanding compact cards");
