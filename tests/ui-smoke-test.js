...
assert((appJs.match(/class="panel five-match-controls"/g) || []).length === 1, "5v5 mobile/desktop must reuse one action bar in the DOM");
assert(/@media \(max-width: 780px\)[\s\S]*?\.five-match-controls \{[^}]*position:\s*fixed[\s\S]*?left:\s*0[\s\S]*?right:\s*0[\s\S]*?bottom:\s*0/.test(css), "mobile 5v5 action bar must be fixed to the viewport");
assert(/@media \(max-width: 780px\)[\s\S]*?\.five-match-content \{[^}]*padding-bottom:\s*calc\(var\(--mobile-five-actions-height\) \+ env\(safe-area-inset-bottom\)\)/.test(css), "mobile 5v5 content must compensate the fixed action bar");
