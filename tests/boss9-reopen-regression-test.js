...
api.BossGameOverRuntime.advanceBossRewardMutation({run});
api.BossGameOverRuntime.applyBossVictoryHandoffMutation({run,seasonDb:data,ensureCurrentZoneMutation:()=>api.BossGameOverRuntime.ensureCurrentZoneMutation({run,seasonDb:data,mapEngine:{ensureCurrentZone(current){current.currentZone={bossIndex:current.bossIndex,bossId:String(data.bossOrder[current.bossIndex].teamId),currentNodeId:"zone-next",nodes:[],completedNodeIds:[]};return {generated:true,changed:true};}}}),buildFinalization:()=>{throw Error("boss 9 is not final");}});
