...
const view=c.RoadToGlorySquadView.create({escapeHtml:s=>String(s),playerResolver:resolver,teamEmblemMarkup:(_squad,_side,className)=>`<img class="${className}" src="crest.png" alt="" />`});
...
const html=view.markup(model,{teamName:"THOT-TEAM",teamIdentity:{name:"THOT-TEAM"}});
assert.match(html,/>Run</);assert.match(html,/>Squadra</);assert.match(html,/data-rtg-formation-current/);
assert.doesNotMatch(html,/<small>ROSA<\/small>/);
assert.match(html,/rtg-squad-team-identity/);
assert.match(html,/THOT-TEAM/);
assert.match(html,/crest\.png/);
