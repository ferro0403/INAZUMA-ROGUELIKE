"use strict";
const assert=require("assert"),fs=require("fs"),vm=require("vm");
const c={globalThis:null,Object,Array,String,Number,Math,Set,Map,JSON};c.globalThis=c;vm.createContext(c);
vm.runInContext(fs.readFileSync("js/road-to-glory/rtg-match-view.js","utf8"),c);
const view=c.RoadToGloryMatchView.create({escapeHtml:s=>String(s),userTeamMeta:()=>({name:"Thot-team"}),teamEmblemMarkup:(_s,side)=>`<img class="EMBLEM-${side}" />`});
const p=(id,name,role,overall,category="Normale")=>({playerId:id,name,normalizedRole:role,position:role,overall,category,portraitUrl:""});
const user=p("u1","Beat Scatton","MF",70,"Elite"),opp=p("o1","Arnan Wheelie","MF",74,"Forte");
const match={period:"second_half",status:"active",score:{user:0,opponent:0},possession:"opponent",fieldZone:"midfield",actionIndex:5,actionTarget:20,userSquad:{lineup:[user],bench:[]},opponentSquad:{name:"Svincolati",lineup:[opp],bench:[]},moveUsesByPlayerId:{},pendingEncounter:{minute:13,kind:"midfield",actorSide:"opponent",opponentSide:"user",actorPlayerId:"o1",opponentPlayerId:"u1",userPlayerId:"u1",aiPlayerId:"o1",userKind:"defense",aiKind:"dribble",userBaseActionLabel:"Difesa",normalPreviewProbability:64.2}};
const duel=view.encounterMarkup(match,{userPlayer:user,opponentPlayer:opp});
assert.match(duel,/rtg-duel-versus-board/);
assert.match(duel,/is-opponent-possession/);
assert.doesNotMatch(duel,/rtg-duel-meter-clean/);
assert.doesNotMatch(duel,/rtg-duel-probability-card/);
assert.match(duel,/35\.8%/);
assert.doesNotMatch(duel,/64\.2%/);
assert.match(duel,/DIFESA VS DRIBBLING/);
assert.doesNotMatch(duel,/Beat Scatton difende/i);
assert.doesNotMatch(duel,/Arnan Wheelie prova il dribbling/i);
assert.match(duel,/rarity-elite/);
assert.match(duel,/rarity-forte/);
const shell=view.matchMarkup({...match,pendingEncounter:null,log:[{minute:12,kind:"midfield",zone:"midfield",actorSide:"opponent",actorPlayerId:"o1",opponentPlayerId:"u1",actorWon:true,manual:false,score:{user:0,opponent:0}}]});
assert.match(shell,/rtg-match-polish-v2/);
assert.match(shell,/rtg-match-scoreboard/);
assert.match(shell,/rtg-score-capsule/);
assert.match(shell,/Riepilogo azioni/i);
assert.match(shell,/match-sim-log/);
assert.match(shell,/match-event-marker/);
assert.match(shell,/match-event-kind/);
assert.match(shell,/rtg-live-commandbar/);
assert.match(shell,/Thot-team/);
assert.match(shell,/EMBLEM-user/);
const moveShell=view.matchMarkup({...match,pendingEncounter:null,log:[{minute:18,kind:"dribble",zone:"attack",actorSide:"user",actorPlayerId:"u1",opponentPlayerId:"o1",actorWon:true,manual:true,actorMove:"Fire Tornado",score:{user:0,opponent:0}}]});
assert.match(moveShell,/uses-special-move/);
assert.match(moveShell,/Beat Scatton supera Arnan Wheelie con Fire Tornado/i);
assert.match(moveShell,/Fire Tornado/i);
const failedMoveShell=view.matchMarkup({...match,pendingEncounter:null,log:[{minute:20,kind:"midfield",zone:"midfield",actorSide:"opponent",actorPlayerId:"o1",opponentPlayerId:"u1",actorWon:false,manual:true,actorMove:"Ultra Moon",score:{user:0,opponent:0}}]});
assert.match(failedMoveShell,/Beat Scatton conquista il possesso/i);
assert.doesNotMatch(failedMoveShell,/Ultra Moon/i);
assert.doesNotMatch(failedMoveShell,/uses-special-move/);
const result=view.resolvedEncounterMarkup({userWon:true,probability:35.8,userKind:"defense",aiKind:"dribble",actorSide:"opponent",outcomeLabel:"Palla recuperata",userPlayer:user,opponentPlayer:opp,userChoiceLabel:"Difesa",scoreBefore:{user:0,opponent:0},scoreAfter:{user:0,opponent:0}});
assert.match(result,/Beat Scatton recupera palla/i);
assert.doesNotMatch(result,/35\.8%/);
assert.doesNotMatch(result,/Dribbling/i);
assert.doesNotMatch(result,/Difesa/i);
assert.match(result,/Beat Scatton/);
assert.match(result,/Arnan Wheelie/);
assert.match(result,/is-duel-winner/);
assert.match(result,/is-duel-loser/);
assert.doesNotMatch(result,/rtg-duel-result-solo/);
assert.doesNotMatch(result,/VINCITORE/i);
assert.doesNotMatch(result,/rtg-duel-result-meter/);
assert.doesNotMatch(result,/rtg-duel-result-facts rtg-duel-result-facts--compact/);
const midfieldResult=view.resolvedEncounterMarkup({userWon:false,probability:52,userKind:"midfield",aiKind:"midfield",actorSide:"user",userPlayer:user,opponentPlayer:opp,userChoiceLabel:"Dribbling",scoreBefore:{user:0,opponent:0},scoreAfter:{user:0,opponent:0}});
assert.doesNotMatch(midfieldResult,/Dribbling/i);
assert.doesNotMatch(midfieldResult,/Difesa/i);
const opponentMoveLoss=view.resolvedEncounterMarkup({userWon:true,probability:63,userKind:"midfield",aiKind:"midfield",actorSide:"opponent",userPlayer:user,opponentPlayer:opp,userChoiceLabel:"Dribbling",aiChoiceLabel:"Ultra Moon",aiUsedMove:true,aiMovePower:80,scoreBefore:{user:0,opponent:0},scoreAfter:{user:0,opponent:0}});
assert.match(opponentMoveLoss,/Beat Scatton conquista il possesso/i);
assert.doesNotMatch(opponentMoveLoss,/Ultra Moon/i);
assert.doesNotMatch(opponentMoveLoss,/MOSSA SPECIALE USATA/i);
const opponentMoveWin=view.resolvedEncounterMarkup({userWon:false,probability:37,userKind:"midfield",aiKind:"midfield",actorSide:"opponent",userPlayer:user,opponentPlayer:opp,userChoiceLabel:"Dribbling",aiChoiceLabel:"Ultra Moon",aiUsedMove:true,aiMovePower:80,scoreBefore:{user:0,opponent:0},scoreAfter:{user:0,opponent:0}});
assert.match(opponentMoveWin,/Arnan Wheelie vince il duello a centrocampo con Ultra Moon/i);
assert.match(opponentMoveWin,/MOSSA SPECIALE USATA/i);
const suspense=view.resolvingEncounterMarkup({userKind:"midfield",aiKind:"midfield",actorSide:"opponent",userPlayer:user,opponentPlayer:opp,opponentLabel:"Occult",userChoiceLabel:"Dribbling",aiChoiceLabel:"Ultra Moon",aiUsedMove:true,aiMovePower:80});
assert.match(suspense,/CHI AVRÀ LA MEGLIO/i);
assert.match(suspense,/Ultra Moon/i);
assert.doesNotMatch(suspense,/VINTA|PERSA|AZIONE RIUSCITA|AZIONE PERSA/i);
const css=fs.readFileSync("css/road-to-glory.css","utf8");
assert.match(css,/--rtg-match-paper:\s*#fffdf6/i);
assert.match(css,/\.rtg-match-polish-v2 \.rtg-match-ticker/);
assert.match(css,/\.rtg-duel-versus-board/);
assert.match(css,/\.rtg-duel-meter-clean/);
assert.match(css,/\.rtg-match-revolution \.rtg-live-commandbar/);
assert.match(css,/\.rtg-match-event-feed \.match-event-avatar/);
assert.match(css,/\.rtg-match-result--cabin/);
assert.match(css,/\.rtg-match-squad-field-shell/);
assert.match(css,/\.rtg-match-event-move/);
assert.match(css,/\.rtg-duel-active-move/);
assert.match(css,/\.rtg-duel-resolving/);
assert.match(css,/@keyframes rtg-duel-vs-pulse/);
assert.match(css,/width:calc\(100% \+ 16px\)/);
assert.match(css,/\.rtg-prematch-revolution \.rtg-prematch-matchup/);
assert.match(css,/\.rtg-duel-portrait-panel\.rarity-elite/);
assert.match(css,/--duel-rarity-bg/);
assert.match(css,/--duel-side-accent:var\(--duel-rarity-accent/);
assert.match(css,/\.rtg-duel-choice-card\.move-category--shot/);
assert.match(css,/\.rtg-duel-choice-card\.move-category--defense/);
assert.match(css,/\.rtg-duel-choice-card\.move-category--dribble/);
assert.match(css,/\.rtg-duel-choice-card\.move-category--save/);
assert.match(css,/\.rtg-match-revolution \.rtg-match-scoreboard/);
assert.match(css,/\.rtg-duel-goal-celebration/);
assert.match(css,/\.rtg-prematch-top-match/);
assert.match(css,/\.rtg-duel-result-player\.is-duel-loser/);
assert.match(css,/\.rtg-duel-result-player\.is-duel-winner/);
assert.match(css,/\.rtg-duel-versus-board--result\.rtg-duel-result-solo/);
assert.match(css,/\.rtg-duel-card\.is-user-possession/);
assert.match(css,/\.rtg-duel-card\.is-opponent-possession/);
assert.match(css,/\.rtg-halftime-break-banner/);
assert.match(css,/\.rtg-match-scoreboard \.rtg-abandon-button/);
const controllerSource=fs.readFileSync("js/road-to-glory/rtg-controller.js","utf8");
assert.match(controllerSource,/userUsedMove:choice==="move"/);
assert.match(controllerSource,/aiUsedMove:before\.aiChoice==="move"/);
console.log("rtg-match-professional-ui-test: PASS");
