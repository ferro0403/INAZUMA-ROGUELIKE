...
assert.deepEqual(dev.BUILD_REQUIREMENTS, { Buono:1, Forte:2, Elite:4, Mondiale:4, Leggenda:4 });
...
dev.reset(); dev.addProjectModules("Buono",1); assert.equal(dev.read().projects.Buono,1); assert.equal(dev.read().projectBuild.Buono,0);
dev.reset(); dev.addProjectModules("Forte",1); assert.equal(dev.read().projects.Forte,0); assert.equal(dev.read().projectBuild.Forte,1); dev.addProjectModules("Forte",1); assert.equal(dev.read().projects.Forte,1); assert.equal(dev.read().projectBuild.Forte,0);
...
memory.set(dev.STORAGE_KEY,JSON.stringify({schemaVersion:3,projects:{Elite:1,Mondiale:2},projectBuild:{Elite:1,Mondiale:2}})); const v3=dev.read(); assert.equal(v3.projects.Elite,1); assert.equal(v3.projects.Mondiale,2); assert.equal(v3.projectBuild.Elite,1); assert.equal(v3.projectBuild.Mondiale,2); assert.equal(v3.schemaVersion,5); dev.write(v3); assert.deepEqual(dev.read(),v3);
memory.set(dev.STORAGE_KEY,JSON.stringify({schemaVersion:4,projects:{Forte:3,Elite:2},projectBuild:{Forte:1,Elite:3}})); const v4=dev.read(); assert.equal(v4.projects.Forte,3); assert.equal(v4.projectBuild.Forte,1); assert.equal(v4.projects.Elite,2); assert.equal(v4.projectBuild.Elite,3); dev.write(v4); assert.deepEqual(dev.read(),v4);
assert.equal(dev.SCHEMA_VERSION,5); assert(dev.read().projectBuild);
...
console.log("development-v2-test: V5 migration, Forte 2-module build, warehouse costs and pull idempotency OK");
