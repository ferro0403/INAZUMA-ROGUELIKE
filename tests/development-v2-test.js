...
for (const [from,target,coins,cups,projects] of [[65,"Normale",100,0,0],[70,"Buono",200,1,1],[75,"Forte",400,2,1],[80,"Elite",800,3,2],[85,"Mondiale",1000,5,3],[90,"Leggenda",1500,8,4]]) { dev.reset(); const s=dev.read();s.coins=coins;s.cups=cups;if(projects)s.projects[target]=projects;dev.write(s);const r=dev.evolve({playerId:"p",playerName:"P",basePotential:from,unlocked:true});assert(r.ok);assert.equal(r.target,target); }
dev.reset(); let s=dev.read();s.coins=9999;s.cups=99;s.projects.Mondiale=2;dev.write(s);const before=JSON.stringify(dev.read());assert(!dev.evolve({playerId:"p",basePotential:85,unlocked:true}).ok);assert.equal(JSON.stringify(dev.read()),before);assert.equal(dev.evolve({playerId:"locked",basePotential:70,unlocked:false}).reason,"locked");
dev.reset(); s=dev.read(); s.coins=200; s.cups=1; s.projects.Buono=1; dev.write(s); assert(dev.evolve({playerId:"visible",basePotential:70,unlocked:true}).ok); const source={playerId:"visible",finalOverall:70,maxLevel:20,position:"FW",ratings:{attack:7,control:5,speed:6,grit:5,physical:5,stamina:5,defense:3,save:1}}; const resolved=dev.resolvePlayer(source,20,{compactFormat:{levelMax:20}}); assert.equal(resolved.category,"Buono"); assert.equal(resolved.overall,75); assert.equal(dev.resolvePlayer(source,20,{compactFormat:{levelMax:20}}).category,"Buono","reload keeps permanent rarity");
...
dev.reset(); s=dev.read(); s.coins=9999; s.cups=99; s.projects.Elite=5; dev.write(s); assert(dev.evolve({playerId:"consume",playerName:"Consume",basePotential:80,unlocked:true}).ok); assert.equal(dev.read().projects.Elite,3); assert.deepEqual(dev.projectBuildStatus("Elite"), {rarity:"Elite",required:2,owned:3,complete:1,remainder:1,filled:1,ready:true});
