...
assert.equal(dev.DEVELOPMENT_RESOURCE_ASSETS.coins,"https://dxi4wb638ujep.cloudfront.net/1/k/r/e/rez8i1pp0p8.webp"); assert.equal(dev.DEVELOPMENT_RESOURCE_ASSETS.cups,"assets/development/cup.svg");
const grouped=dev.groupEvolutionHistory([
  {playerId:"p",playerNameSnapshot:"P",fromRarity:"Forte",toRarity:"Elite",timestamp:"2026-01-01T10:00:00Z"},
  {playerId:"q",playerNameSnapshot:"Q",fromRarity:"Forte",toRarity:"Elite",timestamp:"2026-01-01T10:30:00Z"},
  {playerId:"p",playerNameSnapshot:"P",fromRarity:"Elite",toRarity:"Mondiale",timestamp:"2026-01-01T11:00:00Z"},
  {playerId:"p",playerNameSnapshot:"P",fromRarity:"Mondiale",toRarity:"Leggenda",timestamp:"2026-01-01T12:00:00Z"},
]);
assert.equal(grouped.length,2); const pHistory=grouped.find(group=>group.playerId==="p"); assert.equal(pHistory.entries.length,3); assert.equal(pHistory.fromRarity,"Forte"); assert.equal(pHistory.toRarity,"Leggenda"); assert.equal(pHistory.evolutionCount,3);
