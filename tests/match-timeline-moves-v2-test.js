"use strict";
const assert=require("assert"),fs=require("fs"),vm=require("vm");const c={console};c.globalThis=c;c.moveCatalog={seasonId:"ie1",players:{}};c.SeasonRegistry={database:()=>({moveCatalog:c.moveCatalog})};
...
function team(prefix,roles){return{name:prefix,formationId:"4-3-3",players:roles.map((position,i)=>{const playerId=`${prefix}-${i}`,m=position==="GK"?["Keeper Art","save"]:position==="DF"?["Wall Art","defense"]:position==="MF"?["Dribble Art","dribble"]:["Shot Art","shot"];c.moveCatalog.players[playerId]={name:m[0],type:m[1],power:60};return{playerId,name:playerId,position,overall:70,attack:70,control:70,speed:70,grit:70,physical:70,stamina:70,defense:70,save:position==="GK"?70:0};})};}
const roles11=["GK","DF","DF","DF","DF","MF","MF","MF","FW","FW","FW"],u=team("u",roles11),o=team("o",roles11),compat={goal:"shot",shot:"shot",long_shot:"shot",post:"shot",crossbar:"shot",save:"save",defensive_stop:"defense",recovery:"defense",dribble:"dribble"};
