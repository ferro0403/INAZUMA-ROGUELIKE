...
const seasonDb = { seasonId: "ie1", players, formations: { eleven: [{ id: "4-3-3", requirements: { GK:1, DF:4, MF:3, FW:3 } }] }, bossOrder: [{ teamId:"boss", teamName:"Boss", bossLevel:1, startingXIPlayerIds:players.map(p=>p.playerId) }] };
...

{
  ...
  assert.match(rt.seam.getAppMarkup(), /Boss della run/);
  assert.doesNotMatch(rt.seam.getAppMarkup(), /getRun\(\)/);
  ...
  assert.match(rt.modalMarkup, /Ricompensa della run/);
  assert.doesNotMatch(rt.modalMarkup, /getRun\(\)/);
}
...
for (const file of ["js/map/run-map-controller.js","js/map/node-router.js","js/map/trade-node-controller.js"]) {
