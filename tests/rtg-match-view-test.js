...
assert.match(pen,/rtg-penalty-player-card--canonical/);
assert.match(pen,/rtg-penalty-arena/);
assert.doesNotMatch(pen,/Sei al tiro|Sei in porta|La CPU sceglie in segreto|DOVE TIRI\?|DOVE TI TUFFI\?/i);
const penaltyCalls=compactCalls.filter(x=>String(x.options.extraClass||"").includes("rtg-penalty-player-card--canonical"));
assert.strictEqual(penaltyCalls.length,2,"penalties must render both players through the canonical squad/change card renderer");
