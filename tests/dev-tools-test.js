...
global.localStorage = new Storage(); global.location = { search: '?dev=1', hash: '' };
...
assert.equal(Dev.enabled(), true, '?dev=1 activates tools');
global.location.search = ''; global.location.hash = '#?dev=1'; assert.equal(Dev.enabled(), true, '#?dev=1 also activates tools on mobile/hash URLs');
global.location.hash = '#dev=1'; assert.equal(Dev.enabled(), true, '#dev=1 also activates tools');
global.location.search = '?dev=1'; global.location.hash = ''; Dev.enable(); assert.equal(localStorage.getItem(Dev.ENABLE_KEY), '1'); Dev.disable(); assert.equal(localStorage.getItem(Dev.ENABLE_KEY), null, 'disable removes only its flag');
