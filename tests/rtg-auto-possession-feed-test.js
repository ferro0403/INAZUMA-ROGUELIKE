...
const autoMid=feed({minute:12,actorSide:"opponent",actorPlayerId:"o5",opponentPlayerId:"u5",kind:"midfield",actorWon:false,manual:false});
assert.match(autoMid,/possesso resta a Occult/i);
assert.doesNotMatch(autoMid,/conquista il possesso/i);

const autoDribble=feed({minute:18,actorSide:"opponent",actorPlayerId:"o8",opponentPlayerId:"u1",kind:"dribble",actorWon:false,manual:false});
assert.match(autoDribble,/Possesso invariato per Occult/i);
assert.doesNotMatch(autoDribble,/recupera palla/i);

const autoMoveStop=feed({minute:22,actorSide:"opponent",actorPlayerId:"o8",opponentPlayerId:"u1",kind:"dribble",actorWon:false,manual:false,opponentMove:"The Wall"});
...
const manualMid=feed({minute:27,actorSide:"opponent",actorPlayerId:"o5",opponentPlayerId:"u5",kind:"midfield",actorWon:false,manual:true});
assert.match(manualMid,/conquista il possesso/i);

const manualDribble=feed({minute:31,actorSide:"opponent",actorPlayerId:"o8",opponentPlayerId:"u1",kind:"dribble",actorWon:false,manual:true});
