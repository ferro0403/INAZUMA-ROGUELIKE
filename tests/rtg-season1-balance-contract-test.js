...
assert.strictEqual(Object.values(config.mainRewards).reduce((a,b)=>a+b,0),3500);assert(config.mainTeams.every(teamId=>config.mainRewards[teamId]===350));assert.deepStrictEqual(JSON.parse(JSON.stringify(config.secondaryRewards)),[{amount:200,weight:100}]);
