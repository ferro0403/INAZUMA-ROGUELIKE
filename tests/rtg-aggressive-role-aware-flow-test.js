...
continuity.firstHalfTarget=999;continuity.actionTarget=999;continuity.manualIndexes=Array.from({length:20},(_,i)=>i);continuity.possession="user";continuity.fieldZone="shot";
continuity.log.push({kind:"dribble",actorWon:true,actorSide:"user",actorPlayerId:"uf2"});
continuity=E.prepareNext(continuity);assert.strictEqual(continuity.pendingEncounter.kind,"shot");assert.strictEqual(continuity.pendingEncounter.actorPlayerId,"uf2");
let counter=E.createMatch({matchId:"counter",seed:"counter-seed",userSquad:squad("u"),opponentSquad:squad("o")});
counter.firstHalfTarget=999;counter.actionTarget=999;counter.manualIndexes=[0];counter.possession="user";counter.fieldZone="midfield";counter=E.prepareNext(counter);assert.strictEqual(counter.pendingEncounter.kind,"midfield");
const pending=counter.pendingEncounter;counter=E.resolvePendingEncounter(counter,"base");assert.strictEqual(counter.fieldZone,"attack");assert.strictEqual(counter.possession,counter.log.at(-1).actorWon?pending.actorSide:pending.opponentSide);
let fwShots=0,dfShots=0,dfDefenses=0,fwDefenses=0;
for(let i=0;i<120;i++){
 let s=E.createMatch({matchId:`roles-${i}`,seed:`roles-${i}`,userSquad:squad("u"),opponentSquad:squad("o")});s.firstHalfTarget=999;s.actionTarget=999;s.manualIndexes=[0];s.possession="user";s.fieldZone="shot";s=E.prepareNext(s);
 const shooter=s.pendingEncounter.actorPlayerId;if(shooter.includes("f"))fwShots++;if(shooter.includes("d"))dfShots++;
...
assert(fwShots>dfShots,`FW shots ${fwShots} must exceed DF shots ${dfShots}`);assert(dfDefenses>fwDefenses,`DF defenses ${dfDefenses} must exceed FW defenses ${fwDefenses}`);
