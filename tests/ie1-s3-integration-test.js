...
assert.deepStrictEqual(minimums,[72,73,74,75,76,77,78,79,80,81,82]); assert.strictEqual(season.recruitmentRules.pullFreeAgents.maximumFinalOverall,null); assert.strictEqual(season.recruitmentRules.pullFreeAgents.noMaximumPotential,true);
const eligible = (overall,index) => overall >= minimums[index]; assert(eligible(89,0)); assert(eligible(72,0)); assert(!eligible(74,7)); assert(eligible(79,7)); assert(eligible(89,7));
...
for (const file of ['js/profiled-season.js','js/season-registry.js','js/draft.js','js/level-progression.js','js/special-match.js']) vm.runInContext(fs.readFileSync(file,'utf8'),context,{filename:file});
...
const draftPlayers = recruitment.map(e => ({...e,position:e.normalizedRole})); const draftRun={seasonId:'ie1_s3',runId:'draft',draft:{selectedIds:[]}};
const high=draftPlayers.filter(p=>p.finalOverall>=81).slice(0,4); draftRun.draft.selectedIds=high.slice(0,3).map(p=>p.playerId); assert.strictEqual(context.DraftEngine.draftHighOverallCount(draftRun,draftPlayers),3); assert(!context.DraftEngine.canSelectInitialDraftCandidate(draftRun,high[3],draftPlayers)); assert(context.DraftEngine.canSelectInitialDraftCandidate(draftRun,{finalOverall:80},draftPlayers));
