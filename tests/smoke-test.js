...
assert(appJs.includes('data-active-boss-side="${escapeHtml(activeSide)}"') && appJs.includes('activeSide !== "user"') && appJs.includes('activeSide !== "boss"') && appJs.includes('function switchBossMatchTab(side)'), "desktop boss match must keep cached tabbed formations and display one active side at a time");
...
assert.equal(run.currentZone.currentNodeId, firstChoice, "boss defeat must return to the immediate predecessor before game over");
