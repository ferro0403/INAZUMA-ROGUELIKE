...
assert(css.includes('.pitch-row[data-row-count="4"]'), "tactical rows must size four-player lines explicitly");
assert(mobileMedia.includes('.pitch-row[data-row-count="4"] { --pitch-card-size: min(82px'), "mobile four-player lines must use the compact widened previous layout size");
assert(mobileMedia.includes("width: min(100%, calc(100vw - 24px))"), "mobile player detail modal must fit and center inside viewport");
assert(appJs.includes('data-row-count="${row.ids.length || 1}"'), "squad rows must expose player count to CSS grid");
...
assert(css.includes(".candidate-grid .player-card-large { display: grid; grid-template-columns: 72px minmax(0, 1fr) 44px;"), "mobile pulls must use compact horizontal list cards");
