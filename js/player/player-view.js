(function (global) {
  ...
  function create(deps) {
    ...
    function compactCard(
      ...
      {
        equipment = null,
        equipmentInFooter = false,
        level = player.displayLevelText ?? player.displayLevel ?? 0,
    ...
    function detailMarkup(
      ...
      const sourceFallback = historical
        ? sourcePlayer(playerId, player.recruitmentSource) ||
      ...
      const resolved = historical
        ? {
            ...
            position:
              player.role || player.position || sourceFallback.position || "-",
            element:
              player.element ||
              sourceFallback.element ||
              sourceFallback.type ||
              "-",
            category:
              player.finalRarity ||
              player.category ||
              sourceFallback.category ||
              "Debole",
      ...
      const effectiveStats = historical
        ...
        : albumMode
          ...
          : resolved.baseStats
            ? resolved.stats
            : equipment
              ? applyEquipment(resolved.stats, equipment)
              : resolved.stats;
      const stats = Object.entries(STAT_LABELS)
        .map(([stat, label]) => {
          ...
          return `
            <div class="detail-stat player-stat-card" style="--stat-value:${barValue}%">
              ${statIcon(stat)}
              <span class="detail-stat-label">${label}</span>
              <strong class="detail-stat-value">${effective}</strong>
              ${bonus > 0 ? `<em class="detail-stat-bonus">+${bonus}</em>` : ""}
              <span class="detail-stat-track" aria-hidden="true"><i></i></span>
            </div>`;
        })
        .join("");
      const potential = historical
        ? (resolved.potential ?? "Non disponibile")
        : (resolved.potential ?? player.finalOverall);
      const origin =
        historical && team?.teamName
          ? `<p class="muted player-history-origin">Rosa campione: ${escapeHtml(team.teamName)}${player.recruitmentSource ? ` · Origine: ${escapeHtml(player.recruitmentSource)}` : ""}</p>`
          : "";
      ...
      const moveSeasonId = [seasonId, player.seasonId, player.recruitmentSource, sourceFallback.seasonId, getSeasonId()]
      ...
      const displayLevel = historical
        ? formatLevel(
            player.finalLevel ?? 0,
            player.seasonId || team?.seasonId || team?.modeId,
            player.finalLevelUnits,
          )
        : (player.displayLevelText ??
          formatLevel(level, getSeasonId(), player.displayLevelUnits));
      const contextLabel = albumMode
        ? "Album"
        : historical
          ? "Squadra campione"
          : "";
      return `
        <div class="player-detail-layout ${rarityClass(resolved.category)} ${historical ? "player-detail-historical" : ""}">
          <section class="player-detail-hero ${String(resolved.name || "").length > 18 ? "player-detail-hero--extra-long-name" : String(resolved.name || "").length > 12 ? "player-detail-hero--long-name" : ""}">
            <div class="player-detail-identity">
              ...
              <div class="player-detail-heading">
                ...
                ${albumMode ? `<span class="album-detail-badge">${albumUnlocked ? "SBLOCCATO" : "CONSULTABILE"}</span>` : ""}
              </div>
              <h2 class="player-detail-name">${escapeHtml(resolved.name)}</h2>
              <div class="player-detail-tags">
                <span class="role-chip"><b>${escapeHtml(resolved.position)}</b></span>
                <span class="role-chip detail-element-chip"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3c4 3 7 6 7 10a7 7 0 0 1-14 0c0-4 3-7 7-10Z"/></svg>${escapeHtml(resolved.element || resolved.type || "-")}</span>
                <span class="role-chip">Lv ${escapeHtml(displayLevel)}</span>
              </div>
              <div class="overall-comparison">
                <div><span>Overall<br><b>attuale</b></span><strong>${escapeHtml(resolved.overall ?? "N/D")}</strong></div>
          ...
          <section class="player-detail-content">
            ...
            ${historical ? playerStatsMarkup(team || {}, player, runStats) : ""}
            ${origin}
          </section>
        </div>`;
    }
