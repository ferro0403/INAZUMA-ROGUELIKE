(function (global) {
  ...
  function create({ devMode, getRun, getActiveSeason, getUi, toast }) {
    ...
    function installPersistenceTools() {
      if (!devMode) return;
      global.addEventListener("DOMContentLoaded", () => {
        ...
        const download = (text) => {
          ...
          link.download = `inazuma-raw-save-diagnostic-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
          document.body.appendChild(link);
          link.click();
          link.remove();
          setTimeout(() => URL.revokeObjectURL(link.href), 0);
        };
        const copy = async (value, fallback = false) => {
          ...
          try {
            ...
            if (fallback) download(text);
            else throw error;
            return false;
          } finally {
            console.info("Inazuma persistence report", value);
          }
        };
        tools.querySelector("[data-persistence-diagnostic]").onclick = async () => {
          try { await copy(await global.InazumaPersistenceDiagnostics.snapshot()); }
          finally { setOpen(false); }
        };
        tools.querySelector("[data-raw-save-diagnostic]").onclick = async () => {
          try {
            const copied = await copy(await global.InazumaPersistenceDiagnostics.exportRawLegacySaves(), true);
            feedback.textContent = copied ? "DIAGNOSTICA RAW COPIATA" : "CLIPBOARD NON DISPONIBILE: JSON SCARICATO";
          } finally { setOpen(false); }
        };
        tools.querySelector("[data-persistence-repair]").onclick = async () => {
          try {
            const result = await global.InazumaPersistenceDiagnostics.repair();
            await copy(result);
            alert(repairResultMessage(result));
          } finally { setOpen(false); }
        };
    ...
    function matchDiagnostics() {
      ...
      return {
        ...
        phase: run?.phase,
        simulationState: match?.simulation?.state,
        resolutionApplied: match?.simulation?.resolutionApplied === true,
        result: match?.result,
        winner: match?.simulation?.winner,
        revealedCount: match?.simulation?.revealedCount,
        timelineLength: match?.simulation?.timeline?.length,
        pendingPostMatchAction: match?.pendingPostMatchAction || null,
        lives: run?.lives,
        gameOver: run?.gameOver,
        finalization: run?.finalization?.status || null,
        permanentEffects: {
          pending: effects.filter((effect) => effect.status === "pending").length,
          applied: effects.filter((effect) => effect.status === "applied").length,
        },
        postBossFlow: run?.postBossFlow?.status || null,
      };
    }

    function exposeGlobalDiagnostics() {
      if (!devMode) return;
      global.__INAZUMA_GAMEPLAY_FAILURE_DIAGNOSTICS__ = () => global.RunState.clone(gameplayFailureDiagnostics);
      global.__INAZUMA_MATCH_DIAGNOSTICS__ = matchDiagnostics;
    }

    installPersistenceTools();
    return Object.freeze({ repairResultMessage, recordGameplayFailure, matchDiagnostics, exposeGlobalDiagnostics });
  }

  global.AppDevDiagnostics = Object.freeze({ create });
})(globalThis);
