(function (global) {
  ...
  async function renderAlbumTeams(collectionId = global.AlbumProgress.DEFAULT_COLLECTION_ID) {
    ...
    const teamCards = teams.map((team) => {
      ...
      return `<button type="button" class="panel album-team-card album-team-card--modern ${complete ? "album-complete" : ""}" data-album-team="${view.escapeHtml(team.teamId)}" aria-label="${view.escapeHtml(team.teamName)}: ${view.escapeHtml(progress.unlocked)} su ${view.escapeHtml(progress.total)} giocatori sbloccati, ${view.escapeHtml(percentLabel)}"><span class="album-team-card__stage"><span class="album-team-card__watermark" aria-hidden="true">${albumTeamLogoMarkup(team)}</span><span class="album-team-logo album-team-card__logo">${albumTeamLogoMarkup(team)}</span>${complete ? '<span class="album-team-complete" aria-label="Squadra completa"><span class="album-team-complete__icon" aria-hidden="true">✓</span><span class="album-team-complete__label">COMPLETA</span></span>' : ""}</span><span class="album-team-card__footer"><span class="album-team-card__heading"><strong class="album-team-card__name">${view.escapeHtml(team.teamName)}</strong><span class="album-team-card__open" aria-hidden="true">→</span></span><span class="album-team-card__progress"><span>${view.escapeHtml(progress.unlocked)} / ${view.escapeHtml(progress.total)} sbloccati</span><b>${view.escapeHtml(percentLabel)}</b></span><span class="album-team-card__bar" aria-hidden="true"><span style="width:${percent}%"></span></span></span></button>`;
    }).join("");
