(function (global) {
  ...
  async function whenWritable() { await ready; const recovery = global.PersistenceRecoveryGuard?.getState?.(); if (!global.PersistenceRecoveryGuard?.isBlocked?.() || recovery?.status === "terminal-recovery") return true; await new Promise((resolve) => waiters.add(resolve)); return true; }
