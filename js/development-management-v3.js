(function (global) {
  ...
  function buildRows(state, database, V3) {
    ...
    return Object.entries(state?.players || {}).flatMap(([playerId, chain]) => {
      ...
      return [{ playerId: String(playerId), name: String(base.name || playerId), role: String(base.position || base.role || "—"), base, missingIdentity: false, detailPlayer, activeRarity, activePotential: Number(active.toPotential), path }];
