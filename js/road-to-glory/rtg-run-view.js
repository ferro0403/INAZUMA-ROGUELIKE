(function (global) {
  ...
  function create(deps = {}) {
    ...
    function header(state = {}) {
      return `<header class="topbar rtg-main-topbar"><button type="button" class="btn rtg-home-button" data-rtg-home>← Home</button><div class="rtg-main-title"><p class="eyebrow">Season 1</p><strong class="brand">Road to Glory</strong></div><div class="status-strip"><span class="status-pill rtg-token-pill">◈ ${escape(Number(state.tokens) || 0)}</span><span class="status-pill lives">♥ ${escape(Number(state.lives) || 0)} vite</span></div></header>`;
    }
    ...
    function runMarkup({state,nodes,seasonDb}={}){const list=Array.from(nodes||[]),currentIndex=list.findIndex((node)=>node.id===state?.currentNodeId),viewState={...(state||{}),currentNodeIndex:currentIndex};const blocks=BLOCKS.map((block)=>{const entries=list.map((node,index)=>({node,index})).filter((entry)=>blockForNode(entry.node).index===block.index);return blockMarkup(block,entries,viewState,seasonDb);}).join("");const currentNode=list[currentIndex],currentLabel=currentNode?.type==="main"?teamName(seasonDb,currentNode.teamId):"Svincolati",complete=!!state?.seasonComplete,cleared=complete?list.length:Math.max(0,currentIndex);return `<main class="screen rtg-run-screen">${header(state)}<div class="content narrow rtg-run-content"><section class="rtg-journey-summary" aria-label="Avanzamento percorso"><div><p class="eyebrow">${complete?"Season 1 completata":"La tua prossima partita"}</p><h1>${complete?"Traguardo raggiunto":escape(currentLabel)}</h1><p>${complete?"Hai completato il percorso della Season 1.":`Tappa ${Math.max(1,currentIndex+1)} di ${list.length} · ${currentNode?.type==="main"?"Sfida principale":"Partita secondaria"}`}</p></div>${!complete&&currentNode?`<button type="button" class="btn btn-yellow" data-rtg-current-node="${escape(currentNode.id)}">Prepara partita <span aria-hidden="true">→</span></button>`:""}<div class="rtg-journey-progress" role="progressbar" aria-label="Tappe completate nel percorso attuale" aria-valuenow="${cleared}" aria-valuemin="0" aria-valuemax="${list.length}"><span style="width:${list.length?cleared/list.length*100:0}%"></span></div></section><section class="panel rtg-run-command"><div><p class="eyebrow">La tua collezione</p><h2>Rinforza la squadra</h2><p class="muted">Nuovi giocatori dalle squadre sconfitte.</p></div><button type="button" class="btn btn-yellow rtg-vending-button" data-rtg-open-vending>Distributore S1 <span>300 ◈</span></button></section><section class="rtg-map" aria-label="Percorso Season 1">${blocks}</section></div>${tabs("run")}</main>`;}
    function albumMarkup({state,entries=[]}={}){
      const cards=Array.from(entries||[]);
      const cardMarkup=cards.map(player=>{
        const cardId=player?.cardId||player?.playerId||player?.id||"";
        return compactPlayerCardMarkup
          ? compactPlayerCardMarkup(player,{level:20,overall:player?.overall??player?.finalOverall,dataAttr:`data-rtg-album-player="${escape(cardId)}" aria-label="Apri scheda di ${escape(player?.name||"giocatore")}"`,extraClass:"squad-player-card rtg-picker-squad-card rtg-album-player-card"})
          : `<button type="button" class="rtg-album-fallback" data-rtg-album-player="${escape(cardId)}"><strong>${escape(player?.name||cardId)}</strong><span>${escape(String(player?.position||player?.role||"").toUpperCase())}</span></button>`;
      }).join("");
      return `<main class="screen rtg-run-screen rtg-album-screen">${header(state)}<div class="content narrow rtg-album-content"><section class="rtg-album-hero"><div><p class="eyebrow">Road to Glory · Collezione permanente</p><h1>Album RTG</h1><p>Qui entrano solo i giocatori che hai realmente trovato nel distributore Road to Glory. Vederli durante una run non li sblocca.</p></div><strong class="rtg-album-count">${escape(cards.length)}<small>SBLOCCATI</small></strong></section>${cards.length?`<section class="rtg-album-grid rtg-picker-grid development-squad-card-scope">${cardMarkup}</section>`:`<section class="panel rtg-album-empty"><p class="eyebrow">Album vuoto</p><h2>Trova il primo giocatore</h2><p class="muted">Usa il distributore: il primo giocatore estratto comparirà qui e resterà nell'album anche nelle run successive.</p><button type="button" class="btn btn-yellow" data-rtg-album-vending>VAI AL DISTRIBUTORE</button></section>`}</div>${tabs("album")}</main>`;
    }
    ...
    return Object.freeze({tabs,lockedMarkup:(...args)=>skinTokens(lockedMarkup(...args)),runMarkup:(...args)=>skinTokens(runMarkup(...args)),albumMarkup:(...args)=>skinTokens(albumMarkup(...args)),requirementsMarkup,nodeModalMarkup,vendingMarkup:(...args)=>skinTokens(vendingMarkup(...args)),pullResultMarkup:(...args)=>skinTokens(pullResultMarkup(...args))});
  }
