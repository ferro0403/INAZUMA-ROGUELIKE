  ...
  function create(deps = {}) {
    ...
    function albumRosterMarkup({team={},entries=[],allEntries=[]}={}){
      ...
      const cards=catalog.map(player=>{
        ...
        const card=compactPlayerCardMarkup?compactPlayerCardMarkup(player,{level:20,overall:player?.overall??player?.finalOverall,dataAttr:isUnlocked?`data-rtg-album-player="${escape(cardId)}"`:"",extraClass:"album-player-card"}):`<div class="album-player-card"><strong>${escape(player?.name||cardId)}</strong></div>`;
