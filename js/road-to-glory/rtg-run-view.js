(function (global) {
  ...
  function create(deps = {}) {
    ...
    function runMarkup({state,nodes,seasonDb,seasonConfig=null}={}){
      ...
      return `<main class="screen rtg-run-screen${seasonNo===2?" rtg-run-screen--s2":""}"${routeStyle}>${header(state)}<div class="content narrow rtg-run-content"><section class="rtg-journey-summary" aria-label="Avanzamento percorso"><div class="rtg-journey-main"><div class="rtg-journey-copy"><p class="eyebrow">${complete?`Season ${seasonNo} completata`:"La tua prossima partita"}</p><h1>${complete?"Traguardo raggiunto":escape(currentLabel)}</h1><p>${complete?`Hai completato il percorso della Season ${seasonNo}.`:`Tappa ${Math.max(1,currentIndex+1)} di ${list.length} · ${currentNode?.type==="main"?(currentNode.special?"Partita speciale":"Sfida principale"):"Partita Svincolati"}`}</p></div>${!complete&&currentNode?`<div class="rtg-journey-opponent" aria-hidden="true">${currentMark}</div>`:""}</div>${!complete&&currentNode?`<button type="button" class="btn btn-yellow" data-rtg-current-node="${escape(currentNode.id)}">Prepara partita <span aria-hidden="true">→</span></button>`:""}${transition}<div class="rtg-journey-progress" role="progressbar" aria-label="Tappe completate nel percorso attuale" aria-valuenow="${cleared}" aria-valuemin="0" aria-valuemax="${list.length}"><span style="width:${list.length?cleared/list.length*100:0}%"></span></div></section><section class="panel rtg-run-command"><div><p class="eyebrow">La tua collezione</p><h2>Rinforza la squadra</h2><p class="muted">Nuovi giocatori dalle squadre sconfitte.</p></div><button type="button" class="btn btn-yellow rtg-vending-button" data-rtg-open-vending>Distributore S${seasonNo} <span>300 ◈</span></button></section><section class="rtg-map rtg-map--season-${seasonNo}" aria-label="Percorso Season ${seasonNo}">${blocks}</section>${complete&&sid==="ie1"?`<section class="rtg-season-complete-footer"><div><p class="eyebrow">Season 1 completata</p><strong>Continua il viaggio</strong><span>La Season 2 riparte dalla tua squadra attuale.</span></div><button type="button" class="btn btn-yellow rtg-season-transition rtg-season-transition--footer" data-rtg-enter-season2>ENTRA NELLA SEASON 2 <span aria-hidden="true">→</span></button></section>`:""}</div>${tabs("run")}</main>`;
    }
    function albumCollectionMarkup({state={},unlocked=0,total=0}={}){
      ...
      return `<main class="album-screen album-collections-screen rtg-album-collections-screen"><header class="topbar album-topbar album-collections-topbar"><button type="button" class="btn section-root-button album-collections-home-button" data-rtg-home aria-label="Torna a Road to Glory"><span aria-hidden="true">←</span></button><div class="album-collections-heading"><p class="eyebrow">ALBUM</p><h1>COLLEZIONI</h1></div><span class="album-collections-topbar-spacer" aria-hidden="true"></span></header><section class="album-collection-grid"><button type="button" class="panel album-collection-card" data-rtg-album-collection="${escape(String(state?.activeSeasonId||"ie1"))}" aria-label="Apri collezione Inazuma Eleven ${String(state?.activeSeasonId||"ie1")==="ie1_s2"?"2":"1"}: ${escape(safeUnlocked)} su ${escape(safeTotal)} giocatori sbloccati, ${escape(percent)}%"><span class="album-collection-cover album-collection-cover--hero"><img src="${escape(RTG_ALBUM_COVER_URL)}" alt="" style="object-position:center" loading="lazy" decoding="async" onerror="this.hidden=true; this.parentElement.classList.add('is-fallback');"></span><span class="album-collection-content album-collection-content--hero"><span class="album-collection-title">Inazuma Eleven ${String(state?.activeSeasonId||"ie1")==="ie1_s2"?"2":"1"}</span><span class="album-collection-progress-copy"><span>${escape(safeUnlocked)} / ${escape(safeTotal)} giocatori sbloccati</span><strong>${escape(percent)}%</strong></span><span class="album-collection-progress-bar" aria-hidden="true"><span style="width:${percent}%"></span></span><span class="album-collection-action">Apri collezione <span aria-hidden="true">→</span></span></span></button></section></main>`;
    }
    function albumTeamsMarkup({state={},teams=[]}={}){
