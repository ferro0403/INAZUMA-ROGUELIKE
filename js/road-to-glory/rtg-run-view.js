(function (global) {
  ...
  function create(deps = {}) {
    ...
    function runMarkup({state,nodes,seasonDb,seasonConfig=null}={}){
      ...
      const blocks=BLOCKS.map((block)=>{const entries=list.map((node,index)=>({node,index})).filter((entry)=>blockForNode(entry.node).index===block.index);return blockMarkup(block,entries,viewState,seasonDb);}).join("");
      ...
      return `<main class="screen rtg-run-screen rtg-run-screen--s${seasonNo}"${routeStyle}>${header(state)}<div class="content narrow rtg-run-content"><section class="rtg-journey-summary" aria-label="Avanzamento percorso"><div class="rtg-journey-main"><div class="rtg-journey-copy"><p class="eyebrow">${complete?`Season ${seasonNo} completata`:"La tua prossima partita"}</p><h1>${complete?"Traguardo raggiunto":escape(currentLabel)}</h1><p>${complete?`Hai completato il percorso della Season ${seasonNo}.`:`Tappa ${Math.max(1,currentIndex+1)} di ${list.length} · ${currentNode?.type==="main"?(currentNode.special?"Partita speciale":"Sfida principale"):"Partita Svincolati"}`}</p></div>${!complete&&currentNode?`<div class="rtg-journey-opponent" aria-hidden="true">${currentMark}</div>`:""}</div>${!complete&&currentNode?`<button type="button" class="btn btn-yellow" data-rtg-current-node="${escape(currentNode.id)}">Prepara partita <span aria-hidden="true">→</span></button>`:""}${transition}<div class="rtg-journey-progress" role="progressbar" aria-label="Tappe completate nel percorso attuale" aria-valuenow="${cleared}" aria-valuemin="0" aria-valuemax="${list.length}"><span style="width:${list.length?cleared/list.length*100:0}%"></span></div></section><section class="panel rtg-run-command"><div><p class="eyebrow">La tua collezione</p><h2>Rinforza la squadra</h2><p class="muted">Nuovi giocatori dalle squadre sconfitte.</p></div><button type="button" class="btn btn-yellow rtg-vending-button" data-rtg-open-vending>Distributore S${seasonNo} <span>300 ◈</span></button></section><section class="rtg-map rtg-map--season-${seasonNo}" aria-label="Percorso Season ${seasonNo}">${blocks}</section></div>${tabs("run")}</main>`;
    }
