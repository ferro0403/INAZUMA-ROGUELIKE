(function (global) {
  ...
  function create(deps = {}) {
    ...
    function runMarkup({state,nodes,seasonDb}={}){const list=Array.from(nodes||[]),currentIndex=list.findIndex((node)=>node.id===state?.currentNodeId),viewState={...(state||{}),currentNodeIndex:currentIndex};const blocks=BLOCKS.map((block)=>{const entries=list.map((node,index)=>({node,index})).filter((entry)=>blockForNode(entry.node).index===block.index);return blockMarkup(block,entries,viewState,seasonDb);}).join("");const currentNode=list[currentIndex],currentLabel=currentNode?.type==="main"?teamName(seasonDb,currentNode.teamId):"Svincolati",complete=!!state?.seasonComplete,cleared=complete?list.length:Math.max(0,currentIndex);const currentMark=currentNode?.type==="main"?emblem(currentNode.teamId):`<span class="rtg-journey-free-agent" aria-hidden="true"><svg viewBox="0 0 40 40"><path d="M20 3 34 9v12c0 8-14 16-14 16S6 29 6 21V9Z" fill="currentColor"/><path d="m20 12 7 5-3 8h-8l-3-8Z" fill="#fff"/></svg></span>`;return `<main class="screen rtg-run-screen">${header(state)}<div class="content narrow rtg-run-content"><section class="rtg-journey-summary" aria-label="Avanzamento percorso"><div class="rtg-journey-main"><div class="rtg-journey-copy"><p class="eyebrow">${complete?"Season 1 completata":"La tua prossima partita"}</p><h1>${complete?"Traguardo raggiunto":escape(currentLabel)}</h1><p>${complete?"Hai completato il percorso della Season 1.":`Tappa ${Math.max(1,currentIndex+1)} di ${list.length} · ${currentNode?.type==="main"?"Sfida principale":"Partita secondaria"}`}</p></div>${!complete&&currentNode?`<div class="rtg-journey-opponent" aria-hidden="true">${currentMark}</div>`:""}</div>${!complete&&currentNode?`<button type="button" class="btn btn-yellow" data-rtg-current-node="${escape(currentNode.id)}">Prepara partita <span aria-hidden="true">→</span></button>`:""}<div class="rtg-journey-progress" role="progressbar" aria-label="Tappe completate nel percorso attuale" aria-valuenow="${cleared}" aria-valuemin="0" aria-valuemax="${list.length}"><span style="width:${list.length?cleared/list.length*100:0}%"></span></div></section><section class="panel rtg-run-command"><div><p class="eyebrow">La tua collezione</p><h2>Rinforza la squadra</h2><p class="muted">Nuovi giocatori dalle squadre sconfitte.</p></div><button type="button" class="btn btn-yellow rtg-vending-button" data-rtg-open-vending>Distributore S1 <span>300 ◈</span></button></section><section class="rtg-map" aria-label="Percorso Season 1">${blocks}</section></div>${tabs("run")}</main>`;}
    ...
    function albumCollectionMarkup({unlocked=0,total=0}={}){
      const safeTotal=Math.max(0,Number(total)||0),safeUnlocked=Math.max(0,Number(unlocked)||0);
      const percent=safeTotal?Math.round(safeUnlocked/safeTotal*100):0;
      return `<main class="album-screen album-collections-screen rtg-album-collections-screen"><header class="topbar album-topbar album-collections-topbar"><button type="button" class="btn section-root-button album-collections-home-button" data-rtg-home aria-label="Torna a Road to Glory"><span aria-hidden="true">←</span></button><div class="album-collections-heading"><p class="eyebrow">ALBUM</p><h1>COLLEZIONI</h1></div><span class="album-collections-topbar-spacer" aria-hidden="true"></span></header><section class="album-collection-grid"><button type="button" class="panel album-collection-card" data-rtg-album-collection="ie1" aria-label="Apri collezione Inazuma Eleven 1: ${escape(safeUnlocked)} su ${escape(safeTotal)} giocatori sbloccati, ${escape(percent)}%"><span class="album-collection-cover album-collection-cover--hero"><img src="${escape(RTG_ALBUM_COVER_URL)}" alt="" style="object-position:center" loading="lazy" decoding="async" onerror="this.hidden=true; this.parentElement.classList.add('is-fallback');"></span><span class="album-collection-content album-collection-content--hero"><span class="album-collection-title">Inazuma Eleven 1</span><span class="album-collection-progress-copy"><span>${escape(safeUnlocked)} / ${escape(safeTotal)} giocatori sbloccati</span><strong>${escape(percent)}%</strong></span><span class="album-collection-progress-bar" aria-hidden="true"><span style="width:${percent}%"></span></span><span class="album-collection-action">Apri collezione <span aria-hidden="true">→</span></span></span></button></section></main>`;
    }
