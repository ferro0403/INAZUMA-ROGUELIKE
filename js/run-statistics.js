(function (global) {
  ...
  const MATCH_TYPES = Object.freeze({ FIVE: "five_v_five", BOSS: "boss", SPECIAL: "special_match" });
  ...
  function createEmptyRunStatistics(startedAt = null) {
    return { schemaVersion: SCHEMA_VERSION, startedAt, completedAt: null, durationMs: 0, matchesTotal: 0, winsTotal: 0, lossesTotal: 0, fiveVFiveMatches: 0, fiveVFiveWins: 0, fiveVFiveLosses: 0, bossMatches: 0, bossWins: 0, bossLosses: 0, specialMatches: 0, specialWins: 0, specialLosses: 0, bossAttemptsById: {}, goalsFor: 0, goalsAgainst: 0, goalDifference: 0, cleanSheets: 0, currentWinStreak: 0, longestWinStreak: 0, currentLossStreak: 0, longestLossStreak: 0, livesLost: 0, playersRecruited: 0, recruitmentBySource: {}, bossRewardsChosen: 0, bossRewardsDeclined: 0, pullsOpened: 0, freeAgentPulls: 0, teamPulls: 0, legendaryPulls: 0, pullsDeclined: 0, rerollsUsed: 0, luckyCharmsUsed: 0, itemsObtained: 0, itemsUsed: 0, itemsUsedById: {}, intensiveTrainingUsed: 0, nodesCompleted: 0, nodesCompletedByType: {}, processedMatchIds: {}, processedActionIds: {} };
  }

  function createEmptyPlayerStatistics(playerSnapshot = {}) {
    const id = playerIdOf(playerSnapshot);
    return { playerId: id, firstJoinedAt: null, recruitmentSource: null, recruitedAtLevel: null, recruitedOverall: null, appearances: 0, fiveVFiveAppearances: 0, bossAppearances: 0, specialAppearances: 0, finalAppearances: 0, starts: 0, wins: 0, losses: 0, bossWins: 0, goals: 0, shots: 0, saves: 0, defensiveActions: 0, counterattacks: 0, posts: 0, crossbars: 0, cleanSheets: 0, ratingTotal: 0, ratingCount: 0, averageRating: null, bestRating: null, bestMatchId: null, finalMatchRating: null, finalMatchGoals: 0, finalMatchSaves: 0, finalMatchDefensiveActions: 0, finalLevel: null, finalOverall: null, overallGrowth: null, role: playerRole(playerSnapshot) || null, playerNameSnapshot: playerName(playerSnapshot), portraitUrlSnapshot: playerSnapshot.portraitUrl || playerSnapshot.image || playerSnapshot.imageUrl || null };
  }
  ...
  function ensureRunStatistics(run) {
    ...
    Object.entries(run.playerStatistics).forEach(([playerId, statistics]) => {
      run.playerStatistics[playerId] = { ...createEmptyPlayerStatistics({ playerId }), ...(statistics || {}), playerId: String(playerId) };
    });
  ...
  function applyCompletedMatchStatistics(run, matchSnapshot = {}) {
    ...
    const isSpecial = matchType === MATCH_TYPES.SPECIAL;
    ...
    if (isSpecial) { stats.specialMatches += 1; result === "victory" ? stats.specialWins += 1 : stats.specialLosses += 1; }
    ...
    if (result === "victory") { stats.currentWinStreak += 1; stats.currentLossStreak = 0; stats.longestWinStreak = Math.max(stats.longestWinStreak, stats.currentWinStreak); } else { stats.currentLossStreak += 1; stats.currentWinStreak = 0; stats.longestLossStreak = Math.max(stats.longestLossStreak, stats.currentLossStreak); stats.livesLost += Number(global.RunState?.getLifeDamageForMatch?.(matchType) ?? 1); }
    lineup.forEach((player) => { const ps = ensurePlayerStatistics(run, player); const c = contributions[player.playerId]; ps.appearances += 1; ps.starts += 1; if (isBoss) ps.bossAppearances += 1; else if (isSpecial) ps.specialAppearances += 1; else ps.fiveVFiveAppearances += 1; result === "victory" ? ps.wins += 1 : ps.losses += 1; if (isBoss && result === "victory") ps.bossWins += 1; ps.goals += c.goals; ps.shots += c.shots; ps.saves += c.saves; ps.defensiveActions += c.defensiveActions; ps.counterattacks += c.counterattacks; ps.posts += c.posts; ps.crossbars += c.crossbars; ps.cleanSheets += c.cleanSheets; const rating = calculatePlayerMatchRating(player, c, { result, matchType, goalsAgainst: opponentScore }); c.rating = rating; ps.ratingTotal = round1(numeric(ps.ratingTotal) + rating); ps.ratingCount += 1; ps.averageRating = round1(ps.ratingTotal / ps.ratingCount); if (ps.bestRating == null || rating > ps.bestRating || (rating === ps.bestRating && String(matchId) < String(ps.bestMatchId || "~"))) { ps.bestRating = rating; ps.bestMatchId = matchId; } if (isFinal) { ps.finalAppearances += 1; ps.finalMatchRating = rating; ps.finalMatchGoals = c.goals; ps.finalMatchSaves = c.saves; ps.finalMatchDefensiveActions = c.defensiveActions; } });
    const eventCounts = events.reduce((acc, ev) => { acc[ev.type || "unknown"] = numeric(acc[ev.type || "unknown"]) + 1; return acc; }, {});
    run.matchHistory.push({ matchId, matchType, nodeId: matchSnapshot.nodeId || null, bossId, completedAt: matchSnapshot.completedAt || nowIso(), formation: matchSnapshot.formation || matchSnapshot.formationId || null, userScore, opponentScore, result, userStrength: matchSnapshot.userStrength?.final ?? matchSnapshot.userStrength ?? null, opponentStrength: matchSnapshot.opponentStrength?.final ?? matchSnapshot.opponentStrength ?? null, userChance: matchSnapshot.probabilities?.userChance ?? matchSnapshot.userChance ?? null, lineupIds: Array.from(lineupIds), eventCounts, playerContributions: clone(contributions), forcedOutcome: matchSnapshot.forcedOutcome || matchSnapshot.simulation?.forcedOutcome || null, testControl: Boolean(matchSnapshot.testControl || matchSnapshot.simulation?.testControl) });
