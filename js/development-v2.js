(function (global) {
  ...
  function generateChoices(defeatedBosses, won, random = Math.random) { const table = TABLES[won ? "victory" : Math.min(10, Number(defeatedBosses))]; if (!table || defeatedBosses < 5) return null; const picked = []; ["rare", "advanced", "safe"].forEach((slot) => { const pool = table[slot]; const alternatives = Object.keys(pool).filter((r) => !picked.includes(r)); picked.push(weighted(pool, random, alternatives.length ? picked : [])); }); return picked; }
  ...
  global.DevelopmentV2 = { STORAGE_KEY, SCHEMA_VERSION, RARITIES, PROJECT_RARITIES, COSTS, ASSETS, TABLES, read, write, reset, nextRarity, threshold, weighted, generateChoices, processRunEnd, claimPull, playerUpgrade, evolve };
