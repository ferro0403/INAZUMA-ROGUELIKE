(function (global) {
  ...
  function evolve({ playerId, playerName, basePotential, unlocked, freeAgentEligible }) { if (!unlocked) return {ok:false,reason:"locked"}; if (!freeAgentEligible) return {ok:false,reason:"not_free_agent"}; const state=read(), id=String(playerId), currentPotential=Math.max(Number(basePotential)||0,Number(state.players[id]?.permanentTargetPotential)||0), currentRarity=global.InazumaProgression?.categoryForPotential?.(currentPotential)||RARITIES.filter(r=>threshold(r)<=currentPotential).at(-1), target=nextRarity(currentRarity); if(!target)return{ok:false,reason:"max"}; const cost=COSTS[target], missing={coins:Math.max(0,cost.coins-state.coins),cups:Math.max(0,cost.cups-totalCups(state)),projects:Math.max(0,cost.projects-(state.projects[target]||0))}; if(Object.values(missing).some(Boolean))return{ok:false,reason:"resources",missing}; state.coins-=cost.coins; consumeCups(state,cost.cups); if(cost.projects)state.projects[target]-=cost.projects; const targetPotential=Math.max(currentPotential,threshold(target)), timestamp=new Date().toISOString(); state.players[id]={permanentTargetPotential:targetPotential,permanentPotentialBoost:Math.max(0,targetPotential-Number(basePotential||0)),currentPermanentRarity:target,evolutionCount:Number(state.players[id]?.evolutionCount||0)+1,updatedAt:timestamp}; state.evolutionHistory.unshift({id:`evo_${Date.now()}_${id}`,playerId:id,playerNameSnapshot:playerName||id,fromRarity:currentRarity,toRarity:target,fromPotential:currentPotential,toPotential:targetPotential,projectsConsumed:cost.projects,cupsConsumed:cost.cups,coinsConsumed:cost.coins,timestamp}); return {ok:true,state:write(state),target,targetPotential}; }
  function reset(){return write(empty());}
  global.DevelopmentV2={STORAGE_KEY,SCHEMA_VERSION,SEASON_IDS,RARITIES,PROJECT_RARITIES,PROJECT_PRICES,COSTS,ASSETS,DEVELOPMENT_RESOURCE_ASSETS,read,write,reset,totalCups,consumeCups,processRunEnd,purchaseProject,purchaseEmblem,addCompletedProject,nextRarity,threshold,groupEvolutionHistory,playerUpgrade,optionsFromUpgrade,permanentOptions,resolvePlayer,evolve};
