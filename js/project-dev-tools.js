(function (global) {
  ...
  function devFlagFromLocation() {
    const location = global.location || {};
    const search = String(location.search || "");
    if (new URLSearchParams(search).get("dev") === "1") return true;
    const hash = String(location.hash || "").replace(/^#/, "");
    const query = hash.includes("?") ? hash.slice(hash.indexOf("?") + 1) : hash;
    return new URLSearchParams(query).get("dev") === "1";
  }
  function enabled() { return devFlagFromLocation() || storage()?.getItem(ENABLE_KEY) === "1"; }
  ...
  const api = { ENABLE_KEY, BACKUP_KEY, devFlagFromLocation, enabled, enable, disable, backupAvailable, createBackup, restoreBackup, beginMutation, record, mutate, addCoins, setCoins, adjustProject, completeProject, resetProject, testCertification, completeDevelopment, resetDevelopment };
