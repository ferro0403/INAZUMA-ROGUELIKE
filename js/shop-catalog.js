(function (global) {
  ...
  function build() { const seen = new Set(), catalog = []; for (const seasonId of SECTION_ORDER.slice(1)) { const database = global.SeasonRegistry?.database?.(seasonId); for (const team of database?.teams || []) { const teamId=String(team.teamId||team.id), name=String(team.teamName||team.name||teamId), key=canonicalName(name); if(EXCLUDED[seasonId]?.has(teamId)||(seasonId === "ie1_s3" && seen.has(key)))continue; if (seasonId !== "ie2") seen.add(key); const rarity=SPECIAL[seasonId]?.[teamId]||"base", tier=EMBLEM_TIERS[rarity]; catalog.push(Object.freeze({teamId,name,seasonId,shopSection:seasonId,emblemId:`team:${seasonId}:${teamId}`,rarity,...tier})); } } return Object.freeze(catalog); }
