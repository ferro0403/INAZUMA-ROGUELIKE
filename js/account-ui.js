(function (global) {
  ...
  function cloudSaveMarkup() {
    ...
    if (status === "associated" || status === "synced") return '<section class="account-cloud-save" data-cloud-save-status><strong>SALVATAGGIO SINCRONIZZATO</strong><span>Il progresso di questo dispositivo è aggiornato sul tuo account.</span></section>';
    ...
    if (status === "sync-conflict") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>VERSIONE CLOUD PIÙ RECENTE</strong><span>Le modifiche locali non sono state sovrascritte.</span></section>';
  ...
  function onClick(event) { const target = event.target.closest?.("[data-account-trigger],[data-account-close],[data-account-tab],[data-account-reset],[data-account-logout],[data-account-resend],[data-account-retry-profile],[data-cloud-save-retry],[data-cloud-save-sync-retry],[data-cloud-save-update],[data-cloud-save-restore],[data-cloud-save-open]"); if (!target) return; if (target.matches("[data-account-trigger]")) open(state.status === "authenticated" ? null : "login"); else if (target.matches("[data-account-close]")) close(); else if (target.dataset.accountTab) { activeTab = target.dataset.accountTab; open(activeTab); } else if (target.matches("[data-account-reset]")) resetPassword(); else if (target.matches("[data-account-resend]")) resend(); else if (target.matches("[data-account-retry-profile]")) global.InazumaAccount?.retryProfile(); else if (target.matches("[data-account-logout]")) logout(); else if (target.matches("[data-cloud-save-retry]")) global.InazumaCloudSave?.retryAssociation(); else if (target.matches("[data-cloud-save-sync-retry]")) global.InazumaCloudSave?.retrySync(); else if (target.matches("[data-cloud-save-update]")) global.InazumaCloudSave?.updateFromCloud(); else if (target.matches("[data-cloud-save-restore]")) global.InazumaCloudSave?.restoreCloudSave(); else if (target.matches("[data-cloud-save-open]")) global.InazumaCloudSave?.reloadAfterRestore(); }
