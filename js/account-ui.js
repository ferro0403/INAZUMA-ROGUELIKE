(function (global) {
  ...
  function onClick(event) { const target = event.target.closest?.("[data-account-trigger],[data-account-close],[data-account-tab],[data-account-reset],[data-account-logout],[data-account-resend],[data-account-retry-profile],[data-cloud-save-retry],[data-cloud-save-sync-retry],[data-cloud-save-check],[data-cloud-save-update],[data-cloud-save-restore],[data-cloud-save-open]"); if (!target) return; if (target.matches("[data-account-trigger]")) open(state.status === "authenticated" ? null : "login"); else if (target.matches("[data-account-close]")) close(); else if (target.dataset.accountTab) { activeTab = target.dataset.accountTab; open(activeTab); } else if (target.matches("[data-account-reset]")) resetPassword(); else if (target.matches("[data-account-resend]")) resend(); else if (target.matches("[data-account-retry-profile]")) global.InazumaAccount?.retryProfile(); else if (target.matches("[data-account-logout]")) logout(); else if (target.matches("[data-cloud-save-retry]")) global.InazumaCloudSave?.retryAssociation(); else if (target.matches("[data-cloud-save-sync-retry]")) global.InazumaCloudSave?.retrySync(); else if (target.matches("[data-cloud-save-check]")) global.InazumaCloudSave?.checkForCloudUpdate(); else if (target.matches("[data-cloud-save-update]")) global.InazumaCloudSave?.updateFromCloud(); else if (target.matches("[data-cloud-save-restore]")) global.InazumaCloudSave?.restoreCloudSave(); else if (target.matches("[data-cloud-save-open]")) global.InazumaCloudSave?.reloadAfterRestore(); }
