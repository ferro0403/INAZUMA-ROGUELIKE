(function (global) {
  ...
  function consumeAutomaticUpdateNotice() {
    let notice = null; try { notice = JSON.parse(sessionStorage.getItem(AUTO_UPDATE_NOTICE_KEY) || "null"); } catch (_) {} sessionStorage.removeItem(AUTO_UPDATE_NOTICE_KEY);
    if (!notice?.uid || !notice.revision) return; const root = document.getElementById("toast-root"); if (!root) return;
    const element = document.createElement("div"); element.className = "toast"; element.textContent = "SALVATAGGIO AGGIORNATO DAL CLOUD"; root.appendChild(element); setTimeout(() => element.remove(), 4000);
  }
  ...
  function cloudSaveMarkup() {
    ...
    if (status === "associated" || status === "synced" || status === "already-up-to-date") return `<section class="account-cloud-save" data-cloud-save-status><strong>${status === "already-up-to-date" ? "SALVATAGGIO GIÀ AGGIORNATO" : "SALVATAGGIO SINCRONIZZATO"}</strong><span>${cloudState.realtimeStatus === "error" ? "AGGIORNAMENTO AUTOMATICO NON DISPONIBILE. Usa CONTROLLA AGGIORNAMENTI per verificare manualmente." : "Il progresso di questo dispositivo è aggiornato sul tuo account."}</span><button type="button" class="btn account-secondary" data-cloud-save-check>CONTROLLA AGGIORNAMENTI</button></section>`;
    ...
    if (status === "sync-error") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>SINCRONIZZAZIONE NON RIUSCITA</strong><span>Il progresso locale è al sicuro su questo dispositivo.</span><button type="button" class="btn account-secondary" data-cloud-save-sync-retry>RIPROVA</button></section>';
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", consumeAutomaticUpdateNotice, { once: true }); else consumeAutomaticUpdateNotice();
