(function (global) {
  ...
  function cloudSaveMarkup() {
    ...
    if (status === "checking" || status === "idle") return '<section class="account-cloud-save" data-cloud-save-status><strong>CONTROLLO PROGRESSI ACCOUNT...</strong></section>';
    if (status === "uploading") return '<section class="account-cloud-save" data-cloud-save-status><strong>ASSOCIAZIONE PROGRESSI ACCOUNT...</strong></section>';
    if (status === "awaiting-local-save") return '<section class="account-cloud-save" data-cloud-save-status><strong>NESSUN PROGRESSO ACCOUNT DA ASSOCIARE</strong><span>Questo dispositivo non contiene ancora progressi permanenti. Album, Centro di Sviluppo e Albo d’Oro verranno sincronizzati quando cambieranno.</span></section>';
    if (status === "associated" || status === "synced") return '<section class="account-cloud-save" data-cloud-save-status><strong>PROGRESSI ACCOUNT SINCRONIZZATI</strong><span>Il progresso di questo dispositivo è aggiornato sul tuo account.</span><button type="button" class="btn account-secondary" data-cloud-save-check>CONTROLLA AGGIORNAMENTI</button></section>';
    ...
    if (status === "cloud-available") return '<section class="account-cloud-save" data-cloud-save-status><strong>PROGRESSI ACCOUNT CLOUD DISPONIBILI</strong><span>Puoi scaricare su questo dispositivo il progresso collegato al tuo account.</span><button type="button" class="btn account-secondary" data-cloud-save-restore>RIPRISTINA PROGRESSI ACCOUNT</button></section>';
    if (status === "downloading") return '<section class="account-cloud-save" data-cloud-save-status><strong>DOWNLOAD PROGRESSI ACCOUNT...</strong></section>';
    if (status === "verifying") return '<section class="account-cloud-save" data-cloud-save-status><strong>VERIFICA PROGRESSI ACCOUNT...</strong></section>';
    if (status === "restoring") return '<section class="account-cloud-save" data-cloud-save-status><strong>RIPRISTINO PROGRESSI ACCOUNT...</strong></section>';
    if (status === "restore-repair-needed") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>RIPRISTINO ACCOUNT DA COMPLETARE</strong><span>I progressi permanenti restano protetti fino alla pulizia del journal. Le run locali restano giocabili.</span><button type="button" class="btn account-secondary" data-cloud-save-restore>RIPROVA</button></section>';
    if (status === "restore-terminal-error") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>RECOVERY ACCOUNT SICURA</strong><span>Il vecchio target cloud non era immutabile: il ripristino parziale è stato chiuso senza riprenderlo.</span><span>La sincronizzazione permanente resta in attesa; le run locali restano giocabili.</span><button type="button" class="btn account-secondary" data-cloud-save-restore>NUOVO CONFRONTO</button></section>';
    if (status === "restored") return '<section class="account-cloud-save" data-cloud-save-status><strong>PROGRESSI ACCOUNT RIPRISTINATI</strong><span>Il progresso del tuo account è stato scaricato su questo dispositivo.</span><button type="button" class="btn account-secondary" data-cloud-save-open>APRI I PROGRESSI</button></section>';
    if (status === "local-conflict") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>PROGRESSI ACCOUNT LOCALI RILEVATI</strong><span>Scegli esplicitamente quale versione conservare. Nessuna scelta viene applicata automaticamente.</span><button type="button" class="btn account-primary" data-cloud-conflict-local>USA QUESTO DISPOSITIVO</button><button type="button" class="btn account-secondary" data-cloud-conflict-cloud>USA I PROGRESSI ACCOUNT CLOUD</button></section>';
    if (status === "conflict-confirm-local") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>CONFERMA SOSTITUZIONE CLOUD</strong><span>I progressi permanenti di questo dispositivo sostituiranno quelli cloud. Conferma per continuare.</span><button type="button" class="btn account-primary" data-cloud-conflict-local-confirm>CONFERMA SOSTITUZIONE CLOUD</button></section>';
    if (status === "conflict-confirm-cloud") return '<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>CONFERMA RIPRISTINO CLOUD</strong><span>I progressi permanenti locali verranno sostituiti dalla versione cloud. Le run restano locali e non vengono modificate.</span><button type="button" class="btn account-primary" data-cloud-conflict-cloud-confirm>CONFERMA PROGRESSI CLOUD</button></section>';
    ...
    if (status === "conflict-resolving-cloud") return '<section class="account-cloud-save" data-cloud-save-status><strong>PREPARAZIONE RIPRISTINO...</strong><span>I progressi permanenti locali restano protetti fino alla verifica. Le run non sono coinvolte.</span></section>';
    if (status === "restore-error") {
      ...
      return `<section class="account-cloud-save account-cloud-save-error" data-cloud-save-status><strong>${integrity ? "PROGRESSI ACCOUNT CLOUD NON VALIDI" : "RIPRISTINO NON RIUSCITO"}</strong><span>${integrity ? "I dati non sono stati applicati a questo dispositivo." : "I progressi permanenti locali non sono stati modificati. La run locale non è coinvolta."}</span><span>Dettaglio: ${escapeHtml(cloudState.error || "restore-failed")}</span><span>Settore: ${escapeHtml(cloudState.problemSector || "nessuno")}</span><span>Fase: ${escapeHtml(cloudState.restoreStage || "nessuna")}</span><button type="button" class="btn account-secondary" data-cloud-save-restore>RIPROVA</button></section>`;
    }
