(function (global) {
  ...
  function activeUid() {
    const account = global.InazumaAccount?.getState?.();
    if (account?.status === "authenticated" && account.uid) return String(account.uid);
    const guard = global.PersistenceRecoveryGuard?.getState?.();
    return guard?.uid ? String(guard.uid) : null;
  }
  ...
  async function snapshot() {
    ...
    const guard = global.PersistenceRecoveryGuard?.getState?.() || {};
    const uid = activeUid();
    const journalEntry = uid ? all.find((entry) => entry.key === `inazuma.cloud.restoreJournal.${uid}`) : null;
  ...
  async function repair(options = {}) {
    ...
    const uid = activeUid();
    const journalKey = uid ? `inazuma.cloud.restoreJournal.${uid}` : null;
    ...
    if (journal) {
      ...
      if (journal.targetCloudCommitId && typeof resume === "function") {
        ...
        const metadata = safeJson(global.localStorage.getItem(`inazuma.cloud.association.${uid}`));
        const bound = metadata?.uid === uid && journal.uid === uid;
        const revisionMatches = Number(metadata?.revision) === Number(journal.targetCloudRevision);
        const commitMatches = !journal.targetCloudCommitId || metadata?.cloudCommitId === journal.targetCloudCommitId;
        const shaped = metadata && typeof metadata.sectorHashes === "object" && !Array.isArray(metadata.sectorHashes);
        if (bound && revisionMatches && commitMatches && shaped) { global.localStorage.removeItem(journalKey); global.PersistenceRecoveryGuard?.clearBlocked?.(journal.operationId); action = "cleared-verified-complete-journal"; }
