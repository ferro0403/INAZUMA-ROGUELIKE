(function (global) {
  ...
  const devModeEnabled = () => global.ProjectDevTools?.enabled?.() || false;
  ...
  const devLocationHash = String(global.location?.hash || "").replace(/^#/, "");
  const devLocationQuery = devLocationHash.includes("?") ? devLocationHash.slice(devLocationHash.indexOf("?") + 1) : devLocationHash;
  const devRequestedByUrl = new URLSearchParams(global.location?.search || "").get("dev") === "1" || new URLSearchParams(devLocationQuery).get("dev") === "1";
  if (devRequestedByUrl) global.ProjectDevTools?.enable?.();
  ...
  function navIcon(name) {
    const icons = {
      ...
      project: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m13 2-7 11h5l-1 9 8-12h-5V2Z"/></svg>',
    };
    return icons[name] || "";
  }
    ...
    document.querySelectorAll("[data-nav]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (destination === "map") {
          ...
          run.projectReturnPhase = run.phase === "project" ? (run.projectReturnPhase || "map") : run.phase;
          run.phase = "project";
          global.RunState.save(run);
  ...
  function developmentModesSectionMarkup() {
    ...
    const pending = players.filter((player) => player.pendingStage).length;
    return `<section class="development-modes-section" aria-labelledby="development-modes-title"><div class="development-modes-copy"><span>${coinIconMarkup()}</span><div><p class="eyebrow">CRESCITA PERMANENTE</p><h2 id="development-modes-title">CENTRO DI SVILUPPO</h2><p>Investi le monete delle run e rendi permanenti le certificazioni dei tuoi Progetti.</p></div></div><dl><div><dt>Saldo</dt><dd>${escapeHtml(developmentCenter?.coins || 0)}</dd></div><div><dt>Giocatori</dt><dd>${escapeHtml(players.length)}</dd></div><div><dt>Incompleti</dt><dd>${escapeHtml(pending)}</dd></div>${ready ? `<div class="is-ready"><dt>CompletabilI</dt><dd>${escapeHtml(ready)}</dd></div>` : ""}</dl><button type="button" class="btn btn-yellow" id="open-development-modes">APRI IL CENTRO</button></section>`;
  }
  function projectHomeMarkup(savedRun) {
    ...
    return `<section class="project-home-card anime-panel ${player ? "has-player" : ""}">${player ? `<span class="project-home-portrait"><img src="${escapeHtml(playerPortraitUrl(player))}" alt="" /></span>` : ""}<div><small>GIOCATORE PROGETTO</small>${player ? `<strong>${escapeHtml(player.name)} · ${escapeHtml(player.position)}</strong><span>${escapeHtml(progress.permanentRarityAtRunStart)} → ${escapeHtml(progress.targetRarity)} · ${escapeHtml(progress.progressPercent)}% · Bonus +${escapeHtml(progress.projectBoost)}</span><i><b style="width:${progress.progressPercent}%"></b></i>` : `<strong>Nessun Progetto selezionato</strong><span>Scegli uno svincolato della tua rosa.</span>`}</div><div class="project-home-economy"><span>Boss ${bosses}</span><span>${pending} monete ${redeemable ? "riscattabili" : "in sospeso"}</span></div><button type="button" class="btn btn-yellow" id="open-project-home">${player ? "GESTISCI PROGETTO" : "SCEGLI PROGETTO"}</button></section>`;
  }

  const PROJECT_GROUP_LABELS = { usage: "UTILIZZO E VITTORIE", role: "PRESTAZIONE DEL RUOLO", boss: "BOSS E CONTINUITÀ", final: "FINALE DELLA RUN" };
  function projectObjectiveRows(progress) {
    const stage = global.ProjectConfig.stageForRarity(progress.permanentRarityAtRunStart); const roleNames = { FW: "Punti realizzazione", MF: "Punti regia", DF: "Punti solidità", GK: "Punti protezione" };
    const initialLives = Number(run.initialLives ?? global.RunState?.initialRunLives?.() ?? 2);
    return { usage: [{ label: "Vittorie ufficiali", current: progress.officialWins, target: stage.wins }], role: [{ label: roleNames[progress.role] || "Punti ruolo", current: progress.rolePoints, target: stage.role[progress.role] }], boss: [{ label: "Boss vinti da titolare", current: progress.bossWins, target: stage.bossWins }, ...(stage.bossStreak == null ? [] : [{ label: "Serie massima di Boss", current: progress.maxBossWinStreak, target: stage.bossStreak }]), ...(stage.firstAttempts == null ? [] : [{ label: "Boss vinti al primo tentativo", current: progress.firstAttemptBossWins, target: stage.firstAttempts }]), ...(stage.maxDefeats == null ? [] : [{ label: stage.maxDefeats === 0 ? "Nessuna sconfitta" : `Massimo ${stage.maxDefeats} sconfitta`, current: progress.defeatsAfterProjectStart <= stage.maxDefeats, failed: progress.defeatsAfterProjectStart > stage.maxDefeats }])], final: [{ label: "Vittoria completa della run", current: progress.finalConditions.runWon }, ...(stage.finalStarter ? [{ label: "Titolare nella finale", current: progress.finalConditions.finalStarter }] : []), ...(stage.finalRoster ? [{ label: "Presente nella rosa finale", current: progress.finalConditions.finalRoster }] : []), ...(stage.minLives == null ? [] : [{ label: `Almeno ${stage.minLives} vite residue`, current: progress.finalConditions.livesRemaining, target: stage.minLives, failed: run.lives < stage.minLives }]), ...(stage.allInitialLives ? [{ label: "Tutte le vite iniziali", current: progress.finalConditions.livesRemaining >= initialLives, failed: run.lives < initialLives }] : []), ...(stage.importantStarts == null ? [] : [{ label: `Titolare nelle ultime ${stage.importantStarts} partite importanti`, current: global.ProjectSystem.importantStarts(run, progress.playerId, stage.importantStarts), target: stage.importantStarts }]), ...(stage.noTestTools ? [{ label: "Nessun utilizzo di strumenti di test", current: !run.projectSystem.testToolUsed, failed: run.projectSystem.testToolUsed }] : [])] };
  }
  function projectObjectivesMarkup(progress) {
    const groups = projectObjectiveRows(progress);
    const firstIncomplete = Object.keys(groups).find((key) => groups[key].some((item) => item.target != null ? Number(item.current) < Number(item.target) : !item.current));
    return `<section class="project-objectives-panel"><div class="project-section-title"><p class="eyebrow">PERCORSO DELLO STADIO</p><h2>OBIETTIVI</h2></div><div class="project-objective-groups">${Object.entries(groups).map(([group, rows]) => { const done = rows.filter((item) => item.target != null ? Number(item.current) >= Number(item.target) : Boolean(item.current)).length; return `<details ${group === firstIncomplete ? "open" : ""}><summary><span>${PROJECT_GROUP_LABELS[group]} · ${done}/${rows.length}</span><strong>${escapeHtml(progress.groups?.[group] || 0)}%</strong></summary><div>${rows.map((item) => { const numeric = item.target != null; const completed = numeric ? Number(item.current) >= Number(item.target) : Boolean(item.current); const percent = numeric ? Math.min(100, Number(item.current || 0) / Number(item.target || 1) * 100) : (completed ? 100 : 0); return `<article class="project-objective ${completed ? "is-complete" : ""} ${item.failed ? "is-failed" : ""}"><span class="project-objective-check" aria-label="${completed ? "Completato" : "Non completato"}">${completed ? "✓" : ""}</span><div><strong>${escapeHtml(item.label)}</strong><small>${item.failed ? "NON PIÙ COMPLETABILE IN QUESTA RUN" : (numeric ? `${escapeHtml(item.current)}/${escapeHtml(item.target)}` : (completed ? "Completato" : "Non completato"))}</small>${numeric ? `<i><b style="width:${percent}%"></b></i>` : ""}</div></article>`; }).join("")}</div></details>`; }).join("")}</div></section>`;
  }
  function projectActiveSummaryMarkup(progress) {
    if (!progress) return `<section class="project-empty-panel"><p class="eyebrow">NESSUN PROGETTO ATTIVO</p><h2>NESSUN PROGETTO ATTIVO</h2><p>Scegli uno svincolato presente nella tua rosa.</p><p class="muted">Completando gli obiettivi sblocchi un bonus temporaneo e una certificazione dopo la vittoria.</p></section>`;
    const player = resolvedRosterPlayer(progress.playerId); const entry = rosterEntry(progress.playerId); const pending = developmentCenter.players?.[progress.playerId]?.pendingStage;
    const source = sourcePlayer(entry); const database = global.SeasonRegistry?.isSeasonSource?.(entry?.source) ? (global.SeasonRegistry.database(entry.source) || seasonDb) : freeAgentsDb; const permanent = run.developmentSnapshot?.[String(progress.playerId)] || {}; const permanentBoost = Number(permanent.permanentPotentialBoost || 0);
    const calculate = (boost) => global.InazumaProgression.getPlayerAtLevel(source, Math.floor(Number(entry?.level || 0)), database, { potentialBoost: Number(entry?.potentialBoost || 0) + permanentBoost + boost, currentOverallBoost: Number(entry?.currentOverallBoost || 0) + Number(permanent.permanentCurrentOverallBoost || 0) + boost, potentialBoostApplications: [...(entry?.potentialBoostApplications || []), ...(permanentBoost ? [{ amount: permanentBoost, appliedLevel: 0 }] : []), ...(boost ? [{ amount: boost, appliedLevel: 0 }] : [])] });
    const base = calculate(0); const reachable = calculate(progress.requiredGrowth); const actual = player; const status = run.projectSystem.testToolUsed ? "CERTIFICAZIONE BLOCCATA DAI TEST" : pending ? "SVILUPPO CENTRO INCOMPLETO" : progress.eligibleForCertification ? "PRONTO PER LA CERTIFICAZIONE" : progress.progressPercent ? "OBIETTIVI IN CORSO" : "IN SVILUPPO";
    return `<section class="project-active-panel"><div class="project-active-card">${tacticalMiniPlayer(progress.playerId, { mode: "squad", area: run.lineup.includes(progress.playerId) ? "lineup" : "bench" })}</div><div class="project-active-data"><span class="project-status-badge">${escapeHtml(status)}</span><h2>${escapeHtml(player?.name || progress.playerId)}</h2><p class="project-rarity-line">${escapeHtml(progress.permanentRarityAtRunStart)} → ${escapeHtml(progress.targetRarity)} · ${escapeHtml(progress.role)} · ${escapeHtml(progress.rolePoints)} punti ruolo</p><div class="project-value-grid project-ovr-grid"><span><small>OVR BASE</small><strong>${escapeHtml(base?.overall ?? base?.finalOverall)}</strong></span><span><small>OVR ATTUALE</small><strong>${escapeHtml(actual?.overall ?? actual?.finalOverall)}</strong></span><span><small>OVR RAGGIUNGIBILE</small><strong>${escapeHtml(Math.min(99, reachable?.overall ?? reachable?.finalOverall))}</strong></span></div><div class="project-total-progress"><span><strong>PROGRESSO PROGETTO</strong><b>${escapeHtml(progress.progressPercent)}% · Bonus +${escapeHtml(progress.projectBoost)} su +${escapeHtml(progress.requiredGrowth)}</b></span><i><b style="width:${progress.progressPercent}%"></b></i></div></div></section>`;
  }
  function projectMissingMarkup(progress) {
    const missing = Object.values(projectObjectiveRows(progress)).flat().filter((item) => item.target != null ? Number(item.current) < Number(item.target) : !item.current).sort((a, b) => Number(Boolean(b.failed)) - Number(Boolean(a.failed)));
    const rows = missing.slice(0, 4).map((item) => `<li class="${item.failed ? "is-failed" : ""}">${item.failed ? "Non più completabile: " : ""}${escapeHtml(item.label)}${item.target != null ? ` · ${escapeHtml(Math.max(0, item.target - Number(item.current || 0)))} mancanti` : ""}</li>`).join("");
    return `<section class="project-missing-panel"><p class="eyebrow">COSA MANCA</p>${missing.length ? `<ul>${rows}</ul>${missing.length > 4 ? `<details><summary>MOSTRA TUTTI</summary><ul>${missing.slice(4).map((item) => `<li>${escapeHtml(item.label)}</li>`).join("")}</ul></details>` : ""}` : `<p>Tutti gli obiettivi sono completati. Vinci la run per certificare la crescita.</p>`}</section>`;
  }
  function otherProjectsMarkup(activeId) {
    const others = Object.values(run.projectSystem.players).filter((progress) => progress.playerId !== activeId && (progress.qualifyingMatches || progress.progressPercent));
    if (!others.length) return "";
    return `<section class="project-other-panel"><div class="project-section-title"><p class="eyebrow">PROGRESSO CONSERVATO</p><h2>ALTRI PROGETTI DELLA RUN</h2></div><div>${others.map((progress) => { const player = resolvedRosterPlayer(progress.playerId); return `<article><div>${tacticalMiniPlayer(progress.playerId, { mode: "squad", area: run.lineup.includes(progress.playerId) ? "lineup" : "bench" })}</div><span><strong>${escapeHtml(player?.name || progress.playerId)}</strong><small>${escapeHtml(progress.progressPercent)}% · Bonus +${escapeHtml(progress.projectBoost)} disattivato</small></span><button type="button" class="btn" data-project-reactivate="${escapeHtml(progress.playerId)}">RIATTIVA</button></article>`; }).join("")}</div></section>`;
  }
  function projectCandidateStatus(playerId) {
    const id = String(playerId); const player = sourcePlayer(rosterEntry(id)); const active = run.projectSystem?.activePlayerId === id; const saved = run.projectSystem?.players?.[id];
    if (!freeAgentsById.has(id)) return { eligible: false, state: "team-player", label: "NON SVINCOLATO" };
    const check = global.ProjectSystem.eligibility({ run, player, freeAgents: freeAgentsDb, developmentCenter });
    if (!check.eligible) {
      if (/Centro/.test(check.reason)) return { eligible: false, state: "pending", label: "SVILUPPO CENTRO INCOMPLETO" };
      if (/massima/.test(check.reason)) return { eligible: false, state: "maximum", label: "RARITÀ MASSIMA" };
      return { eligible: false, state: "ineligible", label: `NON IDONEO · ${check.reason}` };
    }
    return { eligible: true, state: active ? "active" : (saved ? "preserved" : "available"), label: active ? "ATTIVO" : (saved ? "PROGRESSO CONSERVATO" : "DISPONIBILE") };
  }
  function decorateProjectTacticalCards(root) {
    root.querySelectorAll("[data-project-candidate]").forEach((card) => { const status = projectCandidateStatus(card.dataset.projectCandidate); const badge = document.createElement("span"); badge.className = "project-card-status"; badge.textContent = status.label; card.append(badge); });
  }
  function renderProjectSelectionPage(preselectedId = null) {
    if (!run) return renderHome(); const formation = formationById(run.formationId); let selectedId = preselectedId && projectCandidateStatus(preselectedId).eligible ? String(preselectedId) : null;
    app.innerHTML = `<main class="project-player-view project-selection-view"><header class="project-page-head"><button type="button" class="section-root-button" data-project-cancel aria-label="Annulla e torna al Progetto"><svg class="section-root-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 5 8.5 12l7 7"/><path d="M9 12h11"/></svg></button><div><p class="eyebrow">SELEZIONE TATTICA</p><h1>SCEGLI IL PROGETTO</h1></div><span>${escapeHtml(formation?.name || run.formationId)}</span></header><div class="project-selection-layout"><section class="project-tactical-panel"><div class="squad-panel-head"><div><p class="eyebrow">UNDICI TITOLARI</p><h2>Campo tattico</h2></div><span>Seleziona uno svincolato idoneo</span></div>${squadPitchMarkup({ mode: "project", selectedId })}</section><aside class="project-selection-side"><section><div class="squad-panel-head"><div><p class="eyebrow">ROSA REALE</p><h2>Panchina</h2></div></div><div class="bench-list squad-bench-list project-bench-list">${benchMarkup({ mode: "project", selectedId })}</div></section><div class="project-selection-summary" data-project-selection-summary><strong>${selectedId ? "SELEZIONATO" : "NESSUNA SELEZIONE"}</strong><span>${selectedId ? escapeHtml(resolvedRosterPlayer(selectedId)?.name) : "Tocca una card disponibile sul campo o in panchina."}</span></div><div class="project-selection-actions"><button type="button" class="btn" data-project-cancel>ANNULLA</button><button type="button" class="btn btn-yellow" data-project-confirm ${selectedId ? "" : "disabled"}>CONFERMA PROGETTO</button></div></aside></div></main>`;
    resetRenderedViewScroll(); const main = app.querySelector("main"); decorateProjectTacticalCards(main);
    main.addEventListener("click", (event) => {
      const cancel = event.target.closest("[data-project-cancel]"); if (cancel) return renderProjectPlayerPage();
      const card = event.target.closest("[data-project-candidate]");
      if (card) { if (card.dataset.projectEligible !== "true") return toast(projectCandidateStatus(card.dataset.projectCandidate).label, "error"); selectedId = String(card.dataset.projectCandidate); main.querySelectorAll("[data-project-candidate]").forEach((candidate) => { const chosen = candidate === card; candidate.classList.toggle("is-project-selected", chosen); candidate.setAttribute("aria-pressed", chosen ? "true" : "false"); candidate.querySelector(".project-selected-label")?.remove(); }); const label = document.createElement("span"); label.className = "project-selected-label"; label.textContent = "SELEZIONATO"; card.append(label); const summary = main.querySelector("[data-project-selection-summary]"); summary.innerHTML = `<strong>SELEZIONATO</strong><span>${escapeHtml(resolvedRosterPlayer(selectedId)?.name)}</span>`; main.querySelector("[data-project-confirm]").disabled = false; return; }
      if (event.target.closest("[data-project-confirm]") && selectedId) { const player = freeAgentsById.get(selectedId); const result = global.ProjectSystem.select(run, player, { freeAgents: freeAgentsDb, developmentCenter }); if (!result.eligible) return toast(result.reason, "error"); global.RunState.save(run); toast(result.previousPlayerId && result.previousPlayerId !== selectedId ? "Progetto cambiato: progresso precedente conservato" : "Giocatore Progetto selezionato"); renderProjectPlayerPage(); }
    });
  }
  function renderProjectPlayerPage() {
    if (!run) return renderHome(); closeModal({ invokeOnClose: false }); ensureRunSchema(); const state = global.ProjectSystem.migrateRun(run); const progress = state.players[state.activePlayerId];
    run.phase = "project"; global.RunState.save(run);
    app.innerHTML = `<main class="screen project-player-view">${topbar("Giocatore Progetto")}<header class="project-page-head"><div><p class="eyebrow">RUN ATTIVA · ${escapeHtml(seasonDisplayName(run.seasonId))}</p><h1>GIOCATORE PROGETTO</h1><p>Completa gli obiettivi della run e certifica la crescita del tuo svincolato.</p></div></header><div class="project-page-layout"><div>${projectActiveSummaryMarkup(progress)}${progress ? projectMissingMarkup(progress) : ""}</div><div>${progress ? projectObjectivesMarkup(progress) : `<section class="project-explainer"><h2>SVILUPPA UNO SVINCOLATO</h2><p>Il bonus cresce insieme agli obiettivi completati.</p></section>`}</div></div><div class="project-actions"><button type="button" class="btn btn-yellow" data-project-open-selection>${progress ? "CAMBIA PROGETTO" : "SCEGLI PROGETTO"}</button><button type="button" class="btn" data-project-open-center>VAI AL CENTRO</button></div>${bottomNav("project")}</main>`;
    resetRenderedViewScroll(); bindSectionRootNav(); bindBottomNav(); app.querySelectorAll("[data-project-open-selection]").forEach((button) => button.addEventListener("click", () => renderProjectSelectionPage())); app.querySelector("[data-project-open-center]")?.addEventListener("click", () => { run.developmentCenterReturn = "project"; global.RunState.save(run); renderDevelopmentCenter(state.activePlayerId); });
  }
  function developmentPlayerStatus(player) {
    if (player.pendingStage) return player.pendingStage.investedCoins >= player.pendingStage.totalCost ? "PRONTO DA COMPLETARE" : "IN SVILUPPO";
    return player.history?.length ? "STADIO COMPLETATO" : "NESSUNO SVILUPPO PENDENTE";
  }
  function developmentPlayerDetailMarkup(player) {
    const stage = player.pendingStage; const percent = stage ? Math.round(stage.investedCoins / stage.totalCost * 100) : 100; const remaining = stage ? stage.totalCost - stage.investedCoins : 0; const growth = stage ? stage.certifiedPotential - stage.fromPotential : 0; const nextPoint = stage && growth ? Math.max(0, Math.ceil((stage.unlockedGrowth + 1) * stage.totalCost / growth) - stage.investedCoins) : 0; const source = sourcePlayer(player.playerId);
    return `<article class="development-player-detail ${rarityClass(player.permanentRarity)}" data-development-player="${escapeHtml(player.playerId)}"><div class="development-identity">${source ? playerCard(global.InazumaProgression.getPlayerAtLevel(source, 0, freeAgentsDb), { level: 0 }) : ""}<div><span class="project-status-badge">${developmentPlayerStatus(player)}</span><h2>${escapeHtml(player.displayName || player.playerId)}</h2><p>${escapeHtml(player.role)} · ${escapeHtml(player.permanentRarity)}${stage ? ` → ${escapeHtml(stage.certifiedRarity)}` : ""}</p><p>OVR originale ${escapeHtml(player.originalOverall)} · OVR permanente ${escapeHtml(Math.min(99, player.originalOverall + player.permanentCurrentOverallBoost))}</p><p>Potenziale ${escapeHtml(player.originalPotential)} → ${escapeHtml(player.permanentPotential)}${stage ? ` / ${escapeHtml(stage.certifiedPotential)}` : ""}</p></div></div>${stage ? `<section class="development-investment"><h3>INVESTIMENTO</h3><strong>${escapeHtml(stage.investedCoins)} / ${escapeHtml(stage.totalCost)}</strong><i class="development-progress"><b style="width:${percent}%"></b></i><p>Crescita permanente: +${escapeHtml(stage.unlockedGrowth)} / +${escapeHtml(growth)}</p><p>Costo rimanente: ${escapeHtml(remaining)} · Prossimo punto tra ${escapeHtml(nextPoint)} monete</p><div class="development-invest-actions">${[10,50,100].map((amount) => `<button type="button" data-invest="${amount}" data-player="${escapeHtml(player.playerId)}">+${amount}</button>`).join("")}<button type="button" data-invest="max" data-player="${escapeHtml(player.playerId)}">MAX</button></div><details><summary>ALTRO IMPORTO</summary><label>Importo <input type="number" min="1" max="${escapeHtml(remaining)}" data-custom-invest></label><button type="button" class="btn" data-invest="custom" data-player="${escapeHtml(player.playerId)}">INVESTI</button></details></section>` : `<section class="development-complete"><strong>SVILUPPO COMPLETATO</strong><p>LA NUOVA RARITÀ È ORA PERMANENTE.</p></section>`}<section class="development-history"><h3>STORICO</h3>${(player.history || []).map((item) => `<p>${escapeHtml(item.fromRarity)} → ${escapeHtml(item.certifiedRarity)} · completato</p>`).join("") || `<p>Nessuno stadio completato.</p>`}</section></article>`;
  }
  function renderDevelopmentCenter(preferredPlayerId = null) {
    closeModal({ invokeOnClose: false }); developmentCenter = global.DevelopmentCenter.load(); const players = Object.values(developmentCenter.players); const activeProjectId = run?.projectSystem?.activePlayerId; let selectedId = String(preferredPlayerId || developmentCenter.selectedPlayerId || (players.some((p) => String(p.playerId) === String(activeProjectId)) ? activeProjectId : "") || players.find((p) => p.pendingStage)?.playerId || players.at(-1)?.playerId || "");
    if (selectedId) { developmentCenter.selectedPlayerId = selectedId; global.DevelopmentCenter.save(developmentCenter); }
    const selected = developmentCenter.players[selectedId]; const fromProject = run?.developmentCenterReturn === "project";
    const selector = players.length > 1 ? `<nav class="development-selector" aria-label="Seleziona giocatore">${players.map((player) => `<button type="button" class="${String(player.playerId) === selectedId ? "is-active" : ""}" data-center-player="${escapeHtml(player.playerId)}"><strong>${escapeHtml(player.displayName)}</strong><small>${escapeHtml(player.permanentRarity)} · ${developmentPlayerStatus(player)}</small></button>`).join("")}</nav>` : "";
    app.innerHTML = `<main class="development-center-screen"><header class="development-center-head"><button type="button" class="section-root-button" data-center-back aria-label="Indietro"><svg class="section-root-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 5 8.5 12l7 7"/><path d="M9 12h11"/></svg></button><div><p class="eyebrow">CRESCITA PERMANENTE</p><h1>Centro di sviluppo</h1></div><div class="development-balance">${coinIconMarkup()}<strong data-center-balance>${escapeHtml(developmentCenter.coins)}</strong><small>monete</small></div></header>${run ? `<p class="development-run-notice">Gli sviluppi completati ora saranno applicati dalla prossima run.</p>` : ""}${selector}<section class="development-detail-host">${selected ? developmentPlayerDetailMarkup(selected) : `<div class="empty-state development-empty"><strong>NESSUN GIOCATORE IN SVILUPPO</strong><span>Vinci una run completando gli obiettivi di un Giocatore Progetto per ottenere una certificazione.</span><button type="button" class="btn btn-yellow" data-center-back>${fromProject ? "TORNA AL PROGETTO" : "TORNA ALLE MODALITÀ"}</button></div>`}</section></main>`;
    resetRenderedViewScroll();
    const goBack = () => fromProject && run ? renderProjectPlayerPage() : renderHome(); app.querySelectorAll("[data-center-back]").forEach((button) => button.addEventListener("click", goBack));
    app.querySelector(".development-selector")?.addEventListener("click", (event) => { const button = event.target.closest("[data-center-player]"); if (!button) return; selectedId = button.dataset.centerPlayer; developmentCenter.selectedPlayerId = selectedId; global.DevelopmentCenter.save(developmentCenter); app.querySelectorAll("[data-center-player]").forEach((item) => item.classList.toggle("is-active", item === button)); app.querySelector(".development-detail-host").innerHTML = developmentPlayerDetailMarkup(developmentCenter.players[selectedId]); });
    app.querySelector(".development-detail-host")?.addEventListener("click", (event) => { const button = event.target.closest("[data-invest]"); if (!button) return; const player = developmentCenter.players[button.dataset.player]; const remaining = player.pendingStage.totalCost - player.pendingStage.investedCoins; const custom = Number(button.closest(".development-player-detail")?.querySelector("[data-custom-invest]")?.value); const amount = button.dataset.invest === "max" ? Math.min(remaining, developmentCenter.coins) : button.dataset.invest === "custom" ? Math.min(custom, remaining) : Math.min(Number(button.dataset.invest), remaining); if (!amount) return toast("Saldo insufficiente", "error"); try { const result = global.DevelopmentCenter.invest(developmentCenter, player.playerId, amount, `ui_${Date.now()}_${player.playerId}`); global.DevelopmentCenter.save(developmentCenter); app.querySelector(".development-detail-host").innerHTML = developmentPlayerDetailMarkup(developmentCenter.players[player.playerId]); app.querySelector("[data-center-balance]").textContent = developmentCenter.coins; toast(result.completed ? "SVILUPPO COMPLETATO · La nuova rarità è ora permanente" : `Investite ${amount} monete`); } catch (error) { toast(error.message, "error"); } });
  }

  function persistDevChanges() { if (run) global.RunState.save(run); global.DevelopmentCenter.save(developmentCenter); }
  function devPlayerPool(filters = {}) {
    const all = [...(seasonDb?.players || []), ...(freeAgentsDb?.players || [])]; const seen = new Set();
    return all.filter((player) => { const id = String(player.playerId); if (seen.has(id)) return false; seen.add(id); const query = String(filters.query || "").toLowerCase(); const team = String(filters.team || "").toLowerCase(); return (!query || player.name.toLowerCase().includes(query) || id.includes(query)) && (!team || String(player.teamName || player.teamId || "").toLowerCase().includes(team)) && (!filters.role || player.position === filters.role) && (!filters.rarity || player.category === filters.rarity) && (!filters.freeOnly || freeAgentsById.has(id)); }).slice(0, 30);
  }
  function devPlayerResultsMarkup(filters) {
    return devPlayerPool(filters).map((player) => compactPlayerCardMarkup(global.InazumaProgression.getPlayerAtLevel(player, 0, freeAgentsById.has(String(player.playerId)) ? freeAgentsDb : seasonDb), { level: 0, overall: Math.max(1, Number(player.finalOverall) - 20), dataAttr: `data-dev-player="${escapeHtml(player.playerId)}"`, extraClass: "dev-player-card" })).join("") || `<p class="muted">Nessun giocatore trovato.</p>`;
  }
  function devBackupMarkup() {
    const backup = global.ProjectDevTools.backupAvailable();
    return backup
      ? `<div class="dev-backup is-ready" data-dev-backup><div><strong>BACKUP DISPONIBILE</strong><span>${escapeHtml(formatDate(backup.createdAt))}</span></div><button type="button" class="btn" data-dev-action="restore-backup">RIPRISTINA</button></div>`
      : `<div class="dev-backup" data-dev-backup><div><strong>Backup automatico</strong><span>Viene creato alla prima modifica.</span></div></div>`;
  }
  function devToolButton(action, label, options = {}) {
    const classes = ["btn", options.primary ? "btn-yellow" : "", options.danger ? "btn-danger" : ""].filter(Boolean).join(" ");
    const disabled = options.disabled ? " disabled" : "";
    const title = options.disabledReason ? ` title="${escapeHtml(options.disabledReason)}"` : "";
    return `<button type="button" class="${classes}" data-dev-action="${escapeHtml(action)}"${disabled}${title}>${escapeHtml(label)}</button>`;
  }
  function devProjectStatusMarkup(active) {
    if (!active) return `<div class="dev-empty-note">Seleziona uno svincolato dalla scheda Giocatori e premi “Scegli come Progetto”.</div>`;
    return `<div class="dev-project-stats"><span><small>Vittorie</small><strong data-dev-stat="officialWins">${escapeHtml(active.officialWins || 0)}</strong></span><span><small>Boss</small><strong data-dev-stat="bossWins">${escapeHtml(active.bossWins || 0)}</strong></span><span><small>Punti ruolo</small><strong data-dev-stat="rolePoints">${escapeHtml(active.rolePoints || 0)}</strong></span><span><small>Bonus</small><strong data-dev-stat="projectBoost">+${escapeHtml(active.projectBoost || 0)}</strong></span></div>`;
  }
  function devToolsDrawerMarkup() {
    const hasRun = Boolean(run);
    const active = run?.projectSystem?.players?.[run.projectSystem.activePlayerId] || null;
    const activeName = active ? (resolvedRosterPlayer(active.playerId)?.name || active.playerId) : "Nessuno";
    const noRunReason = "Avvia prima una run";
    const noProjectReason = "Seleziona prima un Giocatore Progetto";
    const noPendingReason = "Serve uno sviluppo certificato pendente";
    const activePlayerId = String(run?.projectSystem?.activePlayerId || "");
    const activeDevelopment = activePlayerId ? developmentCenter?.players?.[activePlayerId] : null;
    const hasPendingDevelopment = Boolean(activeDevelopment?.pendingStage);
    const runButtonsDisabled = !hasRun;
    const projectButtonsDisabled = !active;
    return `<div class="dev-tools-overlay"><aside class="dev-tools-drawer" role="dialog" aria-modal="true" aria-labelledby="dev-tools-title">
      <header class="dev-tools-header"><div><p class="eyebrow">MODALITÀ SVILUPPATORE</p><h2 id="dev-tools-title">STRUMENTI DI TEST</h2></div><button type="button" class="dev-tools-close" data-dev-close aria-label="Chiudi">×</button></header>
      <div class="dev-tools-status" aria-label="Stato strumenti"><span class="${hasRun ? "is-ready" : "is-blocked"}"><small>RUN</small><strong>${hasRun ? `Boss ${escapeHtml(Number(run.bossIndex || 0) + 1)}` : "NON ATTIVA"}</strong></span><span><small>MONETE</small><strong data-dev-balance>${escapeHtml(developmentCenter?.coins || 0)}</strong></span><span class="${active ? "is-ready" : "is-blocked"}"><small>PROGETTO</small><strong data-dev-active-name>${escapeHtml(activeName)}</strong></span></div>
      <p class="dev-tools-warning">Le azioni di questo pannello sono test e bloccano la certificazione ufficiale della run.</p>
      ${devBackupMarkup()}
      <nav class="dev-tools-tabs" role="tablist" aria-label="Categorie strumenti">
        <button type="button" class="is-active" data-dev-tab="quick" role="tab" aria-selected="true">RAPIDO</button>
        <button type="button" data-dev-tab="players" role="tab" aria-selected="false">GIOCATORI</button>
        <button type="button" data-dev-tab="project" role="tab" aria-selected="false">PROGETTO</button>
        <button type="button" data-dev-tab="run" role="tab" aria-selected="false">RUN</button>
        <button type="button" data-dev-tab="center" role="tab" aria-selected="false">CENTRO</button>
      </nav>
      <div class="dev-tools-panels">
        <section class="dev-tools-panel is-active" data-dev-panel="quick" role="tabpanel">
          <div class="dev-panel-title"><div><p class="eyebrow">AZIONI PRINCIPALI</p><h3>Prova subito le nuove funzioni</h3></div></div>
          <div class="dev-quick-grid">
            <button type="button" class="dev-quick-card" data-dev-coins="500"><span>+500</span><strong>Monete</strong><small>Aggiunge saldo test</small></button>
            ${devToolButton("complete-project", "COMPLETA OBIETTIVI", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}
            ${devToolButton("test-certification", "CREA CERTIFICAZIONE TEST", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}
            ${devToolButton("win-run", "VINCI LA RUN", { primary: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}
            ${devToolButton("open-center", "APRI CENTRO DI SVILUPPO")}

          </div>
          <p class="dev-tools-help">Per scegliere un giocatore usa la scheda <strong>GIOCATORI</strong>. Per alterare obiettivi specifici usa <strong>PROGETTO</strong>.</p>
        </section>
        <section class="dev-tools-panel" data-dev-panel="players" role="tabpanel" hidden>
          <div class="dev-panel-title"><div><p class="eyebrow">ROSA E RICERCA</p><h3>Scegli un giocatore</h3></div><span data-dev-selected-player>Nessuna selezione</span></div>
          <div class="dev-filter-grid"><input type="search" placeholder="Nome o ID" data-dev-player-query><input type="search" placeholder="Squadra" data-dev-team><select data-dev-role><option value="">Tutti i ruoli</option><option>GK</option><option>DF</option><option>MF</option><option>FW</option></select><select data-dev-rarity><option value="">Tutte le rarità</option>${Object.keys(CATEGORY_CLASS_BY_NAME).map((rarity) => `<option>${rarity}</option>`).join("")}</select><label class="dev-checkbox"><input type="checkbox" data-dev-free-only> Solo svincolati</label></div>
          <div class="dev-player-results" data-dev-player-results>${devPlayerResultsMarkup({})}</div>
          <div class="dev-action-grid">${devToolButton("add-player", "AGGIUNGI ALLA ROSA", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("quick-project", "SCEGLI COME PROGETTO", { primary: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("remove-player", "RIMUOVI DALLA ROSA", { danger: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("fill-roster", "RIEMPI SLOT LIBERI", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("force-add-player", "FORZA AGGIUNTA", { danger: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}</div>
        </section>
        <section class="dev-tools-panel" data-dev-panel="project" role="tabpanel" hidden>
          <div class="dev-panel-title"><div><p class="eyebrow">GIOCATORE PROGETTO</p><h3 data-dev-project-title>${active ? `${escapeHtml(activeName)} · ${escapeHtml(active.progressPercent || 0)}%` : "Nessun Progetto attivo"}</h3></div></div>
          <div data-dev-project-status>${devProjectStatusMarkup(active)}</div>
          <div class="dev-action-grid">${[ ["officialWins","+ VITTORIA"], ["bossWins","+ BOSS"], ["rolePoints","+ PUNTO RUOLO"] ].map(([key,label]) => `<button type="button" class="btn" data-dev-project="${key}" ${projectButtonsDisabled ? `disabled title="${noProjectReason}"` : ""}>${label}</button>`).join("")}${devToolButton("complete-project", "COMPLETA TUTTO", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}</div><details class="dev-advanced"><summary>AVANZATE</summary><div class="dev-action-grid">${[ ["firstAttemptBossWins","+ PRIMO TENTATIVO"], ["maxBossWinStreak","+ SERIE"], ["defeatsAfterProjectStart","+ SCONFITTA"] ].map(([key,label]) => `<button type="button" class="btn" data-dev-project="${key}" ${projectButtonsDisabled ? `disabled title="${noProjectReason}"` : ""}>${label}</button>`).join("")}</div>
          <div class="dev-inline-form"><label>Percentuale test<input type="number" min="0" max="100" data-dev-value="project-percent" value="${escapeHtml(active?.progressPercent || 0)}"></label>${devToolButton("set-project-percent", "IMPOSTA %", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}</div></details>
          <div class="dev-action-grid">${devToolButton("final-starter", "TITOLARE FINALE", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("complete-project", "COMPLETA TUTTO", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("test-certification", "CERTIFICAZIONE TEST", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("intensive-training", "ALLENAMENTO +3", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}</div><details class="dev-reset"><summary>RESET</summary>${devToolButton("reset-project", "RESET PROGETTO", { danger: true, disabled: !hasRun, disabledReason: noRunReason })}</details>
        </section>
        <section class="dev-tools-panel" data-dev-panel="run" role="tabpanel" hidden>
          <div class="dev-panel-title"><div><p class="eyebrow">CONTROLLO RUN</p><h3>${hasRun ? escapeHtml(seasonDisplayName(run.seasonId)) : "Nessuna run attiva"}</h3></div></div>
          <div class="dev-form-grid"><label>Vite<input type="number" min="0" max="9" value="${escapeHtml(run?.lives ?? 0)}" data-dev-value="lives" ${runButtonsDisabled ? "disabled" : ""}></label><label>Boss corrente<input type="number" min="0" value="${escapeHtml(run?.bossIndex ?? 0)}" data-dev-value="boss" ${runButtonsDisabled ? "disabled" : ""}></label><label>Boss battuti<input type="number" min="0" max="11" value="${escapeHtml(run?.completedBossIds?.length || 0)}" data-dev-value="boss-wins" ${runButtonsDisabled ? "disabled" : ""}></label><label>Livello squadra<input type="number" min="0" max="20" value="${escapeHtml(run?.teamLevel ?? 0)}" data-dev-value="team-level" ${runButtonsDisabled ? "disabled" : ""}></label></div>
          <div class="dev-action-grid">${devToolButton("set-run", "APPLICA VALORI", { primary: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("next-boss", "BOSS SUCCESSIVO", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("mode-10", "SIMULA 10 BOSS", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("mode-11", "SIMULA 11 BOSS", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("set-team-level", "APPLICA LIVELLO", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("win-run", "VITTORIA IMMEDIATA", { primary: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("lose-run", "SCONFITTA IMMEDIATA", { danger: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}</div>
        </section>
        <section class="dev-tools-panel" data-dev-panel="center" role="tabpanel" hidden>
          <div class="dev-panel-title"><div><p class="eyebrow">CENTRO E INVENTARIO</p><h3>Monete e sviluppo</h3></div><strong data-dev-balance>${escapeHtml(developmentCenter?.coins || 0)}</strong></div>
          <div class="dev-coin-presets">${[10,50,100,500].map((amount) => `<button type="button" class="btn" data-dev-coins="${amount}">+${amount}</button>`).join("")}</div>
          <div class="dev-inline-form"><label>Saldo personalizzato<input type="number" min="0" data-dev-value="coins" value="${escapeHtml(developmentCenter?.coins || 0)}"></label>${devToolButton("set-coins", "IMPOSTA", { primary: true })}</div>
          <div class="dev-action-grid">${devToolButton("open-center", "APRI CENTRO")}${devToolButton("complete-development", "COMPLETA SVILUPPO", { primary: true, disabled: !hasPendingDevelopment, disabledReason: noPendingReason })}${devToolButton("reset-development", "RESET SVILUPPO", { danger: true })}${devToolButton("redeem-test", "RISCATTA MONETE TEST", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("add-item", "AGGIUNGI OGGETTO", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("assign-equipment", "ASSEGNA EQUIP", { disabled: runButtonsDisabled, disabledReason: noRunReason })}${devToolButton("reset-coins", "AZZERA MONETE", { danger: true })}</div>
          <div class="dev-inline-form"><label>Livello giocatore selezionato<input type="number" min="0" max="20" data-dev-value="player-level" ${runButtonsDisabled ? "disabled" : ""}></label>${devToolButton("set-player-level", "APPLICA", { disabled: runButtonsDisabled, disabledReason: noRunReason })}</div>
        </section>
      </div>
      <footer class="dev-tools-footer"><button type="button" class="btn btn-danger" data-dev-action="disable">DISATTIVA STRUMENTI DI TEST</button></footer>
    </aside></div>`;
  }
  function closeDevTools() { document.querySelector(".dev-tools-overlay")?.remove(); document.documentElement.classList.remove("dev-tools-open"); }
  function isDevToolsCloseRequest(event, overlay) {
    if (event.target === overlay) return true;
    const trigger = event.target?.closest?.("[data-dev-close]");
    return Boolean(trigger && trigger !== overlay);
  }
  function setDevToolsTab(overlay, tabName) {
    overlay.querySelectorAll("[data-dev-tab]").forEach((button) => { const active = button.dataset.devTab === tabName; button.classList.toggle("is-active", active); button.setAttribute("aria-selected", active ? "true" : "false"); });
    overlay.querySelectorAll("[data-dev-panel]").forEach((panel) => { const active = panel.dataset.devPanel === tabName; panel.classList.toggle("is-active", active); panel.hidden = !active; });
  }
  function syncDevToolsDrawer(overlay, selectedPlayerId = null) {
    if (!overlay?.isConnected && !document.body.contains?.(overlay)) return;
    overlay.querySelectorAll("[data-dev-balance]").forEach((node) => { node.textContent = String(developmentCenter?.coins || 0); });
    overlay.querySelectorAll('[data-dev-value="coins"]').forEach((input) => { if (document.activeElement !== input) input.value = String(developmentCenter?.coins || 0); });
    const active = run?.projectSystem?.players?.[run.projectSystem.activePlayerId] || null;
    const activeName = active ? (resolvedRosterPlayer(active.playerId)?.name || active.playerId) : "Nessuno";
    overlay.querySelectorAll("[data-dev-active-name]").forEach((node) => { node.textContent = activeName; });
    const projectTitle = overlay.querySelector("[data-dev-project-title]");
    if (projectTitle) projectTitle.textContent = active ? `${activeName} · ${active.progressPercent || 0}%` : "Nessun Progetto attivo";
    const projectStatus = overlay.querySelector("[data-dev-project-status]");
    if (projectStatus) projectStatus.innerHTML = devProjectStatusMarkup(active);
    const projectActions = new Set(["complete-project", "test-certification", "final-starter", "set-project-percent", "intensive-training"]);
    overlay.querySelectorAll("[data-dev-action]").forEach((button) => { if (projectActions.has(button.dataset.devAction)) button.disabled = !active; });
    overlay.querySelectorAll("[data-dev-project]").forEach((button) => { button.disabled = !active; });
    const pendingIds = Object.values(developmentCenter?.players || {}).filter((player) => player?.pendingStage).map((player) => String(player.playerId));
    const selectedPending = selectedPlayerId && pendingIds.includes(String(selectedPlayerId));
    const activePending = run?.projectSystem?.activePlayerId && pendingIds.includes(String(run.projectSystem.activePlayerId));
    overlay.querySelectorAll('[data-dev-action="complete-development"]').forEach((button) => { button.disabled = !(selectedPending || activePending || pendingIds.length === 1); });
    overlay.querySelectorAll('[data-dev-action="open-certification"]').forEach((button) => { button.disabled = run?.projectSystem?.pendingCertification?.status !== "pending"; });
    const selected = overlay.querySelector("[data-dev-selected-player]");
    if (selected) selected.textContent = selectedPlayerId ? (sourcePlayer(selectedPlayerId)?.name || selectedPlayerId) : "Nessuna selezione";
    const backupHost = overlay.querySelector("[data-dev-backup]");
    if (backupHost) backupHost.outerHTML = devBackupMarkup();
  }

  function openDevTools() {
    closeDevTools();
    document.body.insertAdjacentHTML("beforeend", devToolsDrawerMarkup());
    document.documentElement.classList.add("dev-tools-open");
    const overlay = document.querySelector(".dev-tools-overlay");
    if (!overlay) return;
    overlay.querySelector(".dev-tools-close")?.focus({ preventScroll: true });
    let selectedPlayerId = null;
    const refreshResults = () => {
      const results = overlay.querySelector("[data-dev-player-results]"); if (!results) return;
      results.innerHTML = devPlayerResultsMarkup({ query: overlay.querySelector("[data-dev-player-query]")?.value, team: overlay.querySelector("[data-dev-team]")?.value, role: overlay.querySelector("[data-dev-role]")?.value, rarity: overlay.querySelector("[data-dev-rarity]")?.value, freeOnly: overlay.querySelector("[data-dev-free-only]")?.checked });
      if (selectedPlayerId) results.querySelectorAll("[data-dev-player]").forEach((card) => card.classList.toggle("selected", String(card.dataset.devPlayer) === String(selectedPlayerId)));
    };
    overlay.addEventListener("input", (event) => { if (event.target.matches("[data-dev-player-query],[data-dev-team],[data-dev-role],[data-dev-rarity],[data-dev-free-only]")) refreshResults(); });
    overlay.addEventListener("click", (event) => {
      if (isDevToolsCloseRequest(event, overlay)) return closeDevTools();
      const tab = event.target.closest("[data-dev-tab]"); if (tab) return setDevToolsTab(overlay, tab.dataset.devTab);
      const playerCard = event.target.closest("[data-dev-player]");
      if (playerCard) { overlay.querySelectorAll("[data-dev-player]").forEach((card) => card.classList.toggle("selected", card === playerCard)); selectedPlayerId = playerCard.dataset.devPlayer; syncDevToolsDrawer(overlay, selectedPlayerId); return; }
      const coins = event.target.closest("[data-dev-coins]");
      if (coins) { try { global.ProjectDevTools.addCoins(run, developmentCenter, coins.dataset.devCoins); persistDevChanges(); syncDevToolsDrawer(overlay, selectedPlayerId); toast(`${coins.dataset.devCoins} monete test aggiunte`); } catch (error) { toast(error.message, "error"); } return; }
      const project = event.target.closest("[data-dev-project]");
      if (project) { try { global.ProjectDevTools.adjustProject(run, { [project.dataset.devProject]: 1 }); persistDevChanges(); syncDevToolsDrawer(overlay, selectedPlayerId); toast("Progresso test aggiornato"); } catch (error) { toast(error.message, "error"); } return; }
      const action = event.target.closest("[data-dev-action]")?.dataset.devAction; if (!action) return;
      const result = handleDevAction(action, overlay, selectedPlayerId);
      if (result !== "closed") { refreshResults(); syncDevToolsDrawer(overlay, selectedPlayerId); }
    });
  }
  function handleDevAction(action, drawer, selectedPlayerId) {
    const value = (name) => drawer.querySelector(`[data-dev-value="${name}"]`)?.value;
    const mutate = (name, details, callback) => { global.ProjectDevTools.mutate(run, name, details, callback); persistDevChanges(); };
    const runRequired = new Set(["set-run", "next-boss", "win-run", "lose-run", "mode-10", "mode-11", "open-certification", "complete-project", "final-starter", "set-project-percent", "reset-project", "test-certification", "set-team-level", "set-player-level", "add-item", "assign-equipment", "intensive-training", "add-player", "force-add-player", "remove-player", "fill-roster", "quick-project", "redeem-test"]);
    try {
      if (action === "disable") { global.ProjectDevTools.disable(); closeDevTools(); document.querySelector(".dev-tools-fab")?.remove(); return "closed"; }
      if (action === "restore-backup") { if (global.confirm("Ripristinare il backup completo? Le modifiche test successive verranno sostituite.")) { global.ProjectDevTools.restoreBackup(); global.location.reload(); return "closed"; } return; }
      if (runRequired.has(action) && !run) throw new Error("Avvia prima una run per usare questa azione");
      if (action === "set-coins") global.ProjectDevTools.setCoins(run, developmentCenter, value("coins"));
      else if (action === "reset-coins") { if (!global.confirm("Azzerare soltanto le monete del Centro?")) return; global.ProjectDevTools.setCoins(run, developmentCenter, 0); }
      else if (action === "redeem-test") global.ProjectDevTools.addCoins(run, developmentCenter, global.DevelopmentEconomy.redeemableCoins(run?.completedBossIds?.length || 5));
      else if (action === "set-run") mutate("set-run", {}, () => { run.lives = Math.max(0, Number(value("lives"))); run.bossIndex = Math.max(0, Math.min(seasonDb.bossOrder.length - 1, Number(value("boss")))); const wins = Math.max(0, Number(value("boss-wins"))); run.completedBossIds = Array.from({ length: wins }, (_, index) => String(seasonDb.bossOrder[index]?.teamId || `dev_boss_${index + 1}`)); });
      else if (action === "next-boss") mutate("next-boss", {}, () => { run.bossIndex = Math.min(seasonDb.bossOrder.length - 1, Number(run.bossIndex || 0) + 1); });
      else if (action === "win-run") { mutate("win-run", {}, () => { run.completedBossIds = seasonDb.bossOrder.map((boss) => String(boss.teamId)); run.bossIndex = seasonDb.bossOrder.length; }); closeDevTools(); renderSeasonComplete(); return "closed"; }
      else if (action === "lose-run") { mutate("lose-run", {}, () => { run.lives = 0; run.gameOver = true; run.phase = "gameover"; }); closeDevTools(); renderGameOver(); return "closed"; }
      else if (action === "mode-10" || action === "mode-11") mutate(action, {}, () => { const total = action === "mode-10" ? 10 : 11; run.completedBossIds = Array.from({ length: total }, (_, index) => String(seasonDb.bossOrder[index]?.teamId || `dev_boss_${index + 1}`)); });
      else if (action === "open-certification") { if (run.projectSystem?.pendingCertification?.status !== "pending") throw new Error("Nessuna certificazione pending da riaprire"); closeDevTools(); renderProjectCertification(() => renderProjectPlayerPage()); return "closed"; }
      else if (action === "complete-project") global.ProjectDevTools.completeProject(run);
      else if (action === "final-starter") mutate(action, {}, () => { const progress = run.projectSystem?.players?.[run.projectSystem.activePlayerId]; if (!progress) throw new Error("Seleziona prima un Progetto"); progress.finalMatchStarted = true; progress.finalConditions.finalStarter = true; progress.finalConditions.finalRoster = true; global.ProjectSystem.recalculate(run, progress); });
      else if (action === "set-project-percent") mutate(action, { percent: value("project-percent") }, () => { const progress = run.projectSystem?.players?.[run.projectSystem.activePlayerId]; if (!progress) throw new Error("Seleziona prima un Progetto"); progress.progressPercent = Math.max(0, Math.min(100, Number(value("project-percent")))); progress.projectBoost = progress.progressPercent === 100 ? progress.requiredGrowth : Math.floor(progress.requiredGrowth * progress.progressPercent / 100); });
      else if (action === "reset-project") { if (!global.confirm("Azzerare soltanto il Progetto e i suoi progressi nella run?")) return; global.ProjectDevTools.resetProject(run); }
      else if (action === "test-certification") { const player = sourcePlayer(run?.projectSystem?.activePlayerId); if (!player) throw new Error("Seleziona prima un Progetto"); global.ProjectDevTools.testCertification(run, developmentCenter, player); }
      else if (action === "complete-development") { const pendingIds = Object.values(developmentCenter?.players || {}).filter((player) => player?.pendingStage).map((player) => String(player.playerId)); const preferred = [selectedPlayerId, run?.projectSystem?.activePlayerId].find((id) => id && pendingIds.includes(String(id))); const id = preferred || (pendingIds.length === 1 ? pendingIds[0] : null); if (!id) throw new Error("Seleziona un giocatore con sviluppo pendente"); global.ProjectDevTools.completeDevelopment(run, developmentCenter, id); }
      else if (action === "reset-development") { const id = run?.projectSystem?.activePlayerId || selectedPlayerId; if (!id) throw new Error("Seleziona un giocatore"); if (!global.confirm("Azzerare soltanto lo sviluppo di questo giocatore?")) return; global.ProjectDevTools.resetDevelopment(run, developmentCenter, id); }
      else if (action === "open-center") { closeDevTools(); renderDevelopmentCenter(); return "closed"; }
      else if (action === "set-team-level") mutate("team-level", { level: value("team-level") }, () => { const level = Math.max(0, Math.min(20, Number(value("team-level")))); run.teamLevel = level; run.roster.forEach((entry) => { entry.level = level; }); });
      else if (action === "set-player-level") { if (!selectedPlayerId) throw new Error("Seleziona un giocatore"); mutate(action, { playerId: selectedPlayerId }, () => { const entry = rosterEntry(selectedPlayerId); if (!entry) throw new Error("Giocatore non presente in rosa"); entry.level = Math.max(0, Math.min(20, Number(value("player-level")))); }); }
      else if (action === "add-item") mutate("add-item", {}, () => { run.inventory = Array.isArray(run.inventory) ? run.inventory : []; const item = global.SEASON1_CONFIG.itemPool[0]; if (!item) throw new Error("Nessun oggetto disponibile"); run.inventory.push(makeItemInstance(item, "devTools")); });
      else if (action === "assign-equipment") { if (!selectedPlayerId) throw new Error("Seleziona un giocatore"); mutate(action, { playerId: selectedPlayerId }, () => { run.inventory = Array.isArray(run.inventory) ? run.inventory : []; const entry = rosterEntry(selectedPlayerId); const item = run.inventory.find((candidate) => candidate.kind === "equipment") || global.SEASON1_CONFIG.itemPool.find((candidate) => candidate.kind === "equipment"); if (!entry || !item) throw new Error("Giocatore o equipaggiamento non disponibile"); entry.equippedItem = makeItemInstance(item, "devTools"); }); }
      else if (action === "intensive-training") mutate("intensive-training", {}, () => { const entry = rosterEntry(run.projectSystem?.activePlayerId); if (!entry) throw new Error("Seleziona prima un Progetto"); const amount = Number(itemDefinitionById("intensive_training")?.amount || 3); entry.potentialBoostApplications = [...(entry.potentialBoostApplications || []), { amount, appliedLevel: Number(entry.level || 0) }]; entry.potentialBoost = Number(entry.potentialBoost || 0) + amount; entry.currentOverallBoost = Number(entry.currentOverallBoost || 0) + amount; });
      else if (["add-player", "force-add-player"].includes(action)) { if (!selectedPlayerId) throw new Error("Seleziona un giocatore dai risultati"); const exists = rosterEntry(selectedPlayerId); if (exists) throw new Error("Giocatore già in rosa"); if (run.roster.length >= 15 && action !== "force-add-player") throw new Error("Rosa completa: usa FORZA AGGIUNTA solo per test"); if (action === "force-add-player" && !global.confirm("Forzare l’aggiunta oltre i normali limiti della rosa?")) return; mutate(action, { playerId: selectedPlayerId }, () => { run.roster.push({ playerId: String(selectedPlayerId), source: freeAgentsById.has(String(selectedPlayerId)) ? "free_agents" : activeSeason.id, level: run.teamLevel || 0 }); if (run.bench.length < 4) run.bench.push(String(selectedPlayerId)); }); }
      else if (action === "remove-player") { if (!selectedPlayerId) throw new Error("Seleziona un giocatore"); if (!global.confirm("Rimuovere questo giocatore dalla rosa di test?")) return; mutate(action, { playerId: selectedPlayerId }, () => { run.roster = run.roster.filter((entry) => String(entry.playerId) !== String(selectedPlayerId)); run.lineup = run.lineup.filter((id) => String(id) !== String(selectedPlayerId)); run.bench = run.bench.filter((id) => String(id) !== String(selectedPlayerId)); global.ProjectSystem.reconcileRoster(run); }); }
      else if (action === "fill-roster") mutate(action, {}, () => { const owned = new Set(run.roster.map((entry) => String(entry.playerId))); for (const player of freeAgentsDb.players) { if (run.roster.length >= 15) break; if (owned.has(String(player.playerId))) continue; run.roster.push({ playerId: String(player.playerId), source: "free_agents", level: run.teamLevel || 0 }); owned.add(String(player.playerId)); } reconcileSquadRosterState(); });
      else if (action === "quick-project") { if (!selectedPlayerId) throw new Error("Seleziona un giocatore"); const player = freeAgentsById.get(String(selectedPlayerId)); if (!player || !rosterEntry(selectedPlayerId)) throw new Error("Il Progetto deve essere uno svincolato già in rosa"); mutate(action, { playerId: selectedPlayerId }, () => { const result = global.ProjectSystem.select(run, player, { freeAgents: freeAgentsDb, developmentCenter }); if (!result.eligible) throw new Error(result.reason); }); }
      persistDevChanges(); toast("Azione test applicata"); return "ok";
    } catch (error) { toast(error.message, "error"); return "error"; }
  }
  function installDevToolsAccess() {
    const syncButton = () => { const existing = document.querySelector(".dev-tools-fab"); if (!global.ProjectDevTools.enabled()) return existing?.remove(); if (existing) return; const button = document.createElement("button"); button.type = "button"; button.className = "dev-tools-fab"; button.setAttribute("aria-label", "Apri strumenti di test"); button.innerHTML = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14.5 6.5a5 5 0 0 0-6.7 6.7L3 18l3 3 4.8-4.8a5 5 0 0 0 6.7-6.7l-3 3-3-3 3-3Z"/></svg><span>TEST</span>`; button.addEventListener("click", openDevTools); document.body.append(button); };
    syncButton(); new MutationObserver(syncButton).observe(app, { childList: true }); let taps = 0; let lastTap = 0;
    document.addEventListener("click", (event) => { if (!event.target.closest(".home-wordmark,.loading-brand")) return; const time = Date.now(); taps = time - lastTap < 900 ? taps + 1 : 1; lastTap = time; if (taps >= 7) { global.ProjectDevTools.enable(); taps = 0; syncButton(); toast("Strumenti di test attivati"); } });
  }
  ...
  async function renderHome() {
    ...
    document.getElementById("open-project-home")?.addEventListener("click", renderProjectPlayerPage);
  }
  ...
  async function renderSeasonSelect() {
    ...
    document.getElementById("open-development-modes")?.addEventListener("click", renderDevelopmentCenter);
  }
  ...
  async function resumeRun() {
    ...
    if (run.phase === "project") return renderProjectPlayerPage();
  ...
  function tacticalMiniPlayer(id, { mode = "squad", area = "lineup", selectedId = null } = {}) {
    ...
    const projectStatus = mode === "project" ? projectCandidateStatus(id) : null;
    const dataAttr = mode === "trade"
      ...
      : mode === "equip"
        ...
        : mode === "consumable"
          ...
          : mode === "project"
            ? `data-project-candidate="${escapeHtml(id)}" data-project-eligible="${projectStatus.eligible ? "true" : "false"}" data-project-state="${escapeHtml(projectStatus.state)}" data-area="${area}" aria-pressed="${selected ? "true" : "false"}" aria-label="${escapeHtml(player.name)}: ${escapeHtml(projectStatus.label)}"`
    ...
    return compactPlayerCardMarkup(player, {
      ...
      selected,
      dataAttr,
      extraClass: mode === "squad" ? "squad-player-card" : (mode === "trade" ? "trade-player-card" : (mode === "project" ? `squad-player-card project-tactical-card ${projectStatus.eligible ? "is-eligible" : "is-blocked"} ${projectStatus.state === "active" ? "is-project-active" : ""}` : "")),
    });
  }
  ...
  function renderProjectCertification(onComplete) {
    ...
    app.querySelectorAll("[data-certify]").forEach((button) => button.addEventListener("click", () => { const player = sourcePlayer(button.dataset.certify); const record = global.ProjectSystem.completeCertification(run, player.playerId, player, activeSeason?.id || "default"); if (!record) return; global.DevelopmentCenter.registerCertification(developmentCenter, record, player); global.DevelopmentCenter.save(developmentCenter); global.RunState.save(run); toast("Certificazione ottenuta"); app.innerHTML = `<main class="final-celebration-screen project-certification-screen"><section class="final-celebration-panel"><p class="eyebrow">CERTIFICAZIONE REGISTRATA</p><h1>${escapeHtml(player.name)}</h1><p>Investi nel Centro per rendere permanente la crescita certificata.</p><div class="button-row"><button type="button" class="btn btn-yellow" data-certified-center>VAI AL CENTRO</button><button type="button" class="btn" data-certified-continue>CONTINUA</button></div></section></main>`; app.querySelector("[data-certified-center]").addEventListener("click", () => renderDevelopmentCenter(record.playerId)); app.querySelector("[data-certified-continue]").addEventListener("click", onComplete); }));
  }
  ...
  global.__INAZUMA_UI_TEST__ = {
    ...
    devToolsDrawerMarkup,
    handleDevAction,
    isDevToolsCloseRequest,
    setDevToolsTab,
