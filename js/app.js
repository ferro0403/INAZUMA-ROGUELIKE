(function (global) {
  ...
  function projectObjectiveRows(progress) {
    ...
    return { usage: [{ label: "Vittorie ufficiali", current: progress.officialWins, target: stage.wins }], role: [{ label: roleNames[progress.role] || "Punti ruolo", current: progress.rolePoints, target: stage.role[progress.role] }], boss: [{ label: "Boss vinti da titolare", current: progress.bossWins, target: stage.bossWins }, ...(stage.bossStreak == null ? [] : [{ label: "Serie massima di Boss", current: progress.maxBossWinStreak, target: stage.bossStreak }]), ...(stage.firstAttempts == null ? [] : [{ label: "Boss vinti al primo tentativo", current: progress.firstAttemptBossWins, target: stage.firstAttempts }])], final: [{ label: "Vittoria della run", current: progress.finalConditions.runWon }, ...(stage.finalStarter ? [{ label: "Titolare nella finale", current: progress.finalConditions.finalStarter }] : []), ...(stage.finalRoster ? [{ label: "Presente nella rosa finale", current: progress.finalConditions.finalRoster }] : [])] };
  }
  function projectObjectivesMarkup(progress) {
    const groups = projectObjectiveRows(progress);
    return `<section class="project-objectives-panel"><div class="project-section-title"><p class="eyebrow">PERCORSO DELLO STADIO</p><h2>OBIETTIVI DEL PROGETTO</h2></div><div class="project-objective-groups">${Object.entries(groups).map(([group, rows]) => `<details open><summary><span>${PROJECT_GROUP_LABELS[group]}</span><strong>${escapeHtml(progress.groups?.[group] || 0)}%</strong></summary><div>${rows.map((item) => { const numeric = item.target != null; const completed = numeric ? Number(item.current) >= Number(item.target) : Boolean(item.current); const percent = numeric ? Math.min(100, Number(item.current || 0) / Number(item.target || 1) * 100) : (completed ? 100 : 0); return `<article class="project-objective ${completed ? "is-complete" : ""}"><span class="project-objective-check" aria-label="${completed ? "Completato" : "Non completato"}">${completed ? "✓" : ""}</span><div><strong>${escapeHtml(item.label)}</strong><small>${numeric ? `${escapeHtml(item.current)}/${escapeHtml(item.target)}` : (completed ? "Completato" : "Non completato")}</small><i><b style="width:${percent}%"></b></i></div></article>`; }).join("")}</div></details>`).join("")}</div></section>`;
  }
  function projectActiveSummaryMarkup(progress) {
    if (!progress) return `<section class="project-empty-panel"><p class="eyebrow">NESSUNA SCELTA ATTIVA</p><h2>Nessun Progetto selezionato</h2><p>Scegli uno svincolato presente nella formazione per iniziare il suo percorso.</p><button type="button" class="btn btn-yellow" data-project-open-selection>SCEGLI PROGETTO</button></section>`;
    ...
    return `<section class="project-active-panel"><div class="project-active-card">${tacticalMiniPlayer(progress.playerId, { mode: "squad", area: run.lineup.includes(progress.playerId) ? "lineup" : "bench" })}</div><div class="project-active-data"><p class="eyebrow">PROGETTO ATTIVO</p><h2>${escapeHtml(player?.name || progress.playerId)}</h2><div class="project-value-grid"><span><small>OVR effettivo</small><strong>${escapeHtml(player?.overall)}</strong></span><span><small>Potenziale permanente</small><strong>${escapeHtml(progress.permanentPotentialAtRunStart)}</strong></span><span><small>Certificabile</small><strong>${escapeHtml(progress.permanentPotentialAtRunStart + progress.projectBoost)}</strong></span><span><small>Rarità</small><strong>${escapeHtml(progress.permanentRarityAtRunStart)} → ${escapeHtml(progress.targetRarity)}</strong></span><span><small>Bonus attivo</small><strong>+${escapeHtml(progress.projectBoost)}</strong></span><span><small>Punti ruolo</small><strong>${escapeHtml(progress.rolePoints)}</strong></span></div><div class="project-total-progress"><span><strong>Progresso totale</strong><b>${escapeHtml(progress.progressPercent)}%</b></span><i><b style="width:${progress.progressPercent}%"></b></i></div>${pending ? `<p class="project-block-note">Sviluppo Centro incompleto</p>` : ""}<button type="button" class="btn btn-yellow" data-project-open-selection>CAMBIA PROGETTO</button></div><p class="project-active-note">Soltanto il Progetto attivo riceve il bonus. Il progresso degli altri giocatori rimane conservato.</p></section>`;
  }
  ...
  function renderProjectPlayerPage() {
    ...
    app.innerHTML = `<main class="project-player-view"><header class="project-page-head">${sectionRootButton("projectPlayer")}<div><p class="eyebrow">RUN ATTIVA · ${escapeHtml(seasonDisplayName(run.seasonId))}</p><h1>GIOCATORE PROGETTO</h1></div><span>${escapeHtml(formationById(run.formationId)?.name || run.formationId || "Formazione")}</span></header><div class="project-page-layout"><div>${projectActiveSummaryMarkup(progress)}${otherProjectsMarkup(state.activePlayerId)}</div><div>${progress ? projectObjectivesMarkup(progress) : `<section class="project-explainer"><h2>SVILUPPA UNO SVINCOLATO</h2><p>Completa obiettivi reali, vinci la run, certifica la crescita e rendila permanente nel Centro di sviluppo.</p></section>`}</div></div></main>`;
    resetRenderedViewScroll(); app.querySelector("[data-section-root]")?.addEventListener("click", renderHome); app.querySelectorAll("[data-project-open-selection]").forEach((button) => button.addEventListener("click", () => renderProjectSelectionPage())); app.querySelectorAll("[data-project-reactivate]").forEach((button) => button.addEventListener("click", () => renderProjectSelectionPage(button.dataset.projectReactivate)));
  }
      ...
      <div class="dev-tools-panels">
        <section class="dev-tools-panel is-active" data-dev-panel="quick" role="tabpanel">
          ...
          <div class="dev-quick-grid">
            <button type="button" class="dev-quick-card" data-dev-coins="100"><span>+100</span><strong>Monete</strong><small>Aggiunge saldo test</small></button>
            ...
            ${devToolButton("complete-development", "COMPLETA SVILUPPO", { primary: true, disabled: !hasPendingDevelopment, disabledReason: noPendingReason })}
            ${devToolButton("win-run", "VINCI SUBITO LA RUN", { primary: true, disabled: runButtonsDisabled, disabledReason: noRunReason })}
            ${devToolButton("open-center", "APRI CENTRO DI SVILUPPO")}
            ${devToolButton("open-certification", "APRI CERTIFICAZIONE", { disabled: !run?.projectSystem?.pendingCertification, disabledReason: "Nessuna certificazione da aprire" })}
          </div>
        ...
        <section class="dev-tools-panel" data-dev-panel="project" role="tabpanel" hidden>
          ...
          <div class="dev-action-grid">${[ ["officialWins","+ VITTORIA"], ["bossWins","+ BOSS"], ["firstAttemptBossWins","+ PRIMO TENTATIVO"], ["maxBossWinStreak","+ SERIE"], ["defeatsAfterProjectStart","+ SCONFITTA"], ["rolePoints","+ PUNTO RUOLO"] ].map(([key,label]) => `<button type="button" class="btn" data-dev-project="${key}" ${projectButtonsDisabled ? `disabled title="${noProjectReason}"` : ""}>${label}</button>`).join("")}</div>
          <div class="dev-inline-form"><label>Percentuale test<input type="number" min="0" max="100" data-dev-value="project-percent" value="${escapeHtml(active?.progressPercent || 0)}"></label>${devToolButton("set-project-percent", "IMPOSTA %", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}</div>
          <div class="dev-action-grid">${devToolButton("final-starter", "TITOLARE FINALE", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("complete-project", "COMPLETA TUTTO", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("test-certification", "CERTIFICAZIONE TEST", { primary: true, disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("intensive-training", "ALLENAMENTO +3", { disabled: projectButtonsDisabled, disabledReason: noProjectReason })}${devToolButton("reset-project", "RESET PROGETTO", { danger: true, disabled: !hasRun, disabledReason: noRunReason })}</div>
        </section>
