...
function buildLuckyCharmUpgrades(currentCandidates, available, random) {
  ...
  function renderInventory(options = {}) {
    ...
    const equippedMarkup = equipped.length
      ? `<div class="inventory-v2-list inventory-equipped-list">${equipped.map(({ entry, player, resolved, item }) => `
          ...
          </article>`).join("")}</div><p class="inventory-equipment-note">Ogni giocatore può avere un solo equipaggiamento attivo.</p>`
  ...
  function equipItemToEntry(instanceId, entry) {
    ...
    global.RunState.save(run);
    closeModal();
    toast(`${resolveItem(item).name}\nequipaggiata su ${sourcePlayer(entry).name}`);
