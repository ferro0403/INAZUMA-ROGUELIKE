(function (global) {
  ...
  const DEV_MODE = global.ProjectDevTools?.enabled?.() || false;
  ...
  if (new URLSearchParams(global.location?.search || "").get("dev") === "1") global.ProjectDevTools?.enable?.();
  ...
  function devBackupMarkup() { const backup = global.ProjectDevTools.backupAvailable(); return backup ? `<div class="dev-backup is-ready"><strong>BACKUP DISPONIBILE</strong><span>${escapeHtml(formatDate(backup.createdAt))}</span><button type="button" class="btn" data-dev-action="restore-backup">RIPRISTINA BACKUP</button></div>` : `<div class="dev-backup"><strong>Backup automatico</strong><span>Verrà creato prima della prima modifica.</span></div>`; }
  function devToolsDrawerMarkup() {
    const active = run?.projectSystem?.players?.[run.projectSystem.activePlayerId];
    return `<div class="dev-tools-overlay" data-dev-close><aside class="dev-tools-drawer" role="dialog" aria-modal="true" aria-labelledby="dev-tools-title"><header><div><p class="eyebrow">MODALITÀ SVILUPPATORE</p><h2 id="dev-tools-title">STRUMENTI DI TEST</h2></div><button type="button" class="dev-tools-close" data-dev-close aria-label="Chiudi">×</button></header><p class="dev-tools-warning">Le azioni di questo pannello non sono risultati ufficiali.</p>${devBackupMarkup()}<details open><summary>GIOCATORI</summary><div class="dev-tools-section"><div class="dev-filter-grid"><input type="search" placeholder="Nome o ID" data-dev-player-query><input type="search" placeholder="Squadra" data-dev-team><select data-dev-role><option value="">Tutti i ruoli</option><option>GK</option><option>DF</option><option>MF</option><option>FW</option></select><select data-dev-rarity><option value="">Tutte le rarità</option>${Object.keys(CATEGORY_CLASS_BY_NAME).map((rarity) => `<option>${rarity}</option>`).join("")}</select><label><input type="checkbox" data-dev-free-only> Solo svincolati</label></div><div class="dev-player-results" data-dev-player-results>${devPlayerResultsMarkup({})}</div><div class="dev-action-row"><button type="button" class="btn" data-dev-action="add-player">AGGIUNGI</button><button type="button" class="btn btn-yellow" data-dev-action="force-add-player">FORZA AGGIUNTA</button><button type="button" class="btn" data-dev-action="remove-player">RIMUOVI</button><button type="button" class="btn" data-dev-action="fill-roster">RIEMPI SLOT</button><button type="button" class="btn btn-yellow" data-dev-action="quick-project">SCEGLI COME PROGETTO</button></div></div></details><details open><summary>RUN</summary><div class="dev-tools-section"><label>Vite <input type="number" min="0" max="9" value="${escapeHtml(run?.lives ?? 0)}" data-dev-value="lives"></label><label>Boss corrente <input type="number" min="0" value="${escapeHtml(run?.bossIndex ?? 0)}" data-dev-value="boss"></label><label>Boss battuti <input type="number" min="0" max="11" value="${escapeHtml(run?.completedBossIds?.length || 0)}" data-dev-value="boss-wins"></label><div class="dev-action-row"><button type="button" class="btn" data-dev-action="set-run">APPLICA RUN</button><button type="button" class="btn" data-dev-action="next-boss">BOSS SUCCESSIVO</button><button type="button" class="btn btn-yellow" data-dev-action="win-run">VITTORIA IMMEDIATA</button><button type="button" class="btn btn-danger" data-dev-action="lose-run">SCONFITTA RUN</button><button type="button" class="btn" data-dev-action="mode-10">SIMULA 10 BOSS</button><button type="button" class="btn" data-dev-action="mode-11">SIMULA 11 BOSS</button><button type="button" class="btn" data-dev-action="open-certification">APRI CERTIFICAZIONE</button></div></div></details><details open><summary>MONETE</summary><div class="dev-tools-section"><div class="dev-action-row">${[10,50,100,500].map((amount) => `<button type="button" class="btn" data-dev-coins="${amount}">+${amount}</button>`).join("")}</div><label>Saldo personalizzato <input type="number" min="0" data-dev-value="coins" value="${escapeHtml(developmentCenter.coins)}"></label><div class="dev-action-row"><button type="button" class="btn btn-yellow" data-dev-action="set-coins">IMPOSTA</button><button type="button" class="btn btn-danger" data-dev-action="reset-coins">AZZERA MONETE</button><button type="button" class="btn" data-dev-action="redeem-test">RISCATTA MONETE TEST</button></div></div></details><details open><summary>GIOCATORE PROGETTO</summary><div class="dev-tools-section"><p>${active ? `Attivo: <strong>${escapeHtml(resolvedRosterPlayer(active.playerId)?.name || active.playerId)}</strong> · ${escapeHtml(active.progressPercent)}%` : "Nessun Progetto attivo"}</p><div class="dev-action-row"><button type="button" class="btn" data-dev-project="officialWins">+ VITTORIA</button><button type="button" class="btn" data-dev-project="bossWins">+ BOSS</button><button type="button" class="btn" data-dev-project="firstAttemptBossWins">+ PRIMO TENTATIVO</button><button type="button" class="btn" data-dev-project="maxBossWinStreak">+ SERIE</button><button type="button" class="btn" data-dev-project="defeatsAfterProjectStart">+ SCONFITTA</button><button type="button" class="btn" data-dev-project="rolePoints">+ PUNTO RUOLO</button><button type="button" class="btn" data-dev-action="final-starter">TITOLARE FINALE</button><label>Percentuale <input type="number" min="0" max="100" data-dev-value="project-percent"></label><button type="button" class="btn" data-dev-action="set-project-percent">IMPOSTA %</button><button type="button" class="btn btn-yellow" data-dev-action="complete-project">COMPLETA OBIETTIVI</button><button type="button" class="btn" data-dev-action="test-certification">CREA CERTIFICAZIONE TEST</button><button type="button" class="btn btn-danger" data-dev-action="reset-project">RESET SOLO PROGETTO</button></div></div></details><details><summary>CENTRO E PROGRESSIONE</summary><div class="dev-tools-section"><div class="dev-action-row"><button type="button" class="btn" data-dev-action="open-center">APRI CENTRO</button><button type="button" class="btn btn-yellow" data-dev-action="complete-development">COMPLETA SVILUPPO TEST</button><button type="button" class="btn btn-danger" data-dev-action="reset-development">RESET SVILUPPO GIOCATORE</button></div><label>Livello squadra <input type="number" min="0" max="20" value="${escapeHtml(run?.teamLevel ?? 0)}" data-dev-value="team-level"></label><label>Livello giocatore selezionato <input type="number" min="0" max="20" data-dev-value="player-level"></label><div class="dev-action-row"><button type="button" class="btn" data-dev-action="set-team-level">IMPOSTA LIVELLO</button><button type="button" class="btn" data-dev-action="set-player-level">LIVELLO GIOCATORE</button><button type="button" class="btn" data-dev-action="add-item">AGGIUNGI OGGETTO</button><button type="button" class="btn" data-dev-action="assign-equipment">ASSEGNA EQUIPAGGIAMENTO</button><button type="button" class="btn" data-dev-action="intensive-training">ALLENAMENTO INTENSIVO</button></div></div></details><footer><button type="button" class="btn btn-danger" data-dev-action="disable">DISATTIVA STRUMENTI DI TEST</button></footer></aside></div>`;
  }
  ...
  function openDevTools() {
    closeDevTools(); document.body.insertAdjacentHTML("beforeend", devToolsDrawerMarkup()); document.documentElement.classList.add("dev-tools-open"); const overlay = document.querySelector(".dev-tools-overlay"); overlay.querySelector(".dev-tools-close")?.focus({ preventScroll: true }); let selectedPlayerId = null;
    const refreshResults = () => { overlay.querySelector("[data-dev-player-results]").innerHTML = devPlayerResultsMarkup({ query: overlay.querySelector("[data-dev-player-query]").value, team: overlay.querySelector("[data-dev-team]").value, role: overlay.querySelector("[data-dev-role]").value, rarity: overlay.querySelector("[data-dev-rarity]").value, freeOnly: overlay.querySelector("[data-dev-free-only]").checked }); };
    ...
    overlay.addEventListener("click", (event) => { if (event.target === overlay || event.target.closest("[data-dev-close]")) return closeDevTools(); const playerCard = event.target.closest("[data-dev-player]"); if (playerCard) { overlay.querySelectorAll("[data-dev-player]").forEach((card) => card.classList.toggle("selected", card === playerCard)); selectedPlayerId = playerCard.dataset.devPlayer; return; } const coins = event.target.closest("[data-dev-coins]"); if (coins) { global.ProjectDevTools.addCoins(run, developmentCenter, coins.dataset.devCoins); persistDevChanges(); coins.closest("details").querySelector("[data-dev-value=coins]").value = developmentCenter.coins; return toast(`${coins.dataset.devCoins} monete test aggiunte`); } const project = event.target.closest("[data-dev-project]"); if (project) { try { global.ProjectDevTools.adjustProject(run, { [project.dataset.devProject]: 1 }); persistDevChanges(); toast("Progresso test aggiornato"); } catch (error) { toast(error.message, "error"); } return; } const action = event.target.closest("[data-dev-action]")?.dataset.devAction; if (!action) return; handleDevAction(action, overlay, selectedPlayerId); });
  }
  function handleDevAction(action, drawer, selectedPlayerId) {
    ...
    try {
      if (action === "disable") { global.ProjectDevTools.disable(); closeDevTools(); document.querySelector(".dev-tools-fab")?.remove(); return; }
      if (action === "restore-backup") { if (global.confirm("Ripristinare il backup completo? Le modifiche test successive verranno sostituite.")) { global.ProjectDevTools.restoreBackup(); global.location.reload(); } return; }
      ...
      else if (action === "win-run") { mutate("win-run", {}, () => { run.completedBossIds = seasonDb.bossOrder.map((boss) => String(boss.teamId)); run.bossIndex = seasonDb.bossOrder.length; }); closeDevTools(); return renderSeasonComplete(); }
      else if (action === "lose-run") { mutate("lose-run", {}, () => { run.lives = 0; run.gameOver = true; run.phase = "gameover"; }); closeDevTools(); return renderGameOver(); }
      ...
      else if (action === "open-certification") { if (run.projectSystem?.pendingCertification?.status !== "pending") throw new Error("Nessuna certificazione pending da riaprire"); closeDevTools(); return renderProjectCertification(() => renderProjectPlayerPage()); }
      ...
      else if (action === "complete-development") { const id = run?.projectSystem?.activePlayerId || selectedPlayerId; global.ProjectDevTools.completeDevelopment(run, developmentCenter, id); }
      ...
      else if (action === "open-center") { closeDevTools(); return renderDevelopmentCenter(); }
      ...
      else if (action === "add-item") mutate("add-item", {}, () => { const item = global.SEASON1_CONFIG.itemPool[0]; run.inventory.push(makeItemInstance(item, "devTools")); });
      else if (action === "assign-equipment") { if (!selectedPlayerId) throw new Error("Seleziona un giocatore"); mutate(action, { playerId: selectedPlayerId }, () => { const entry = rosterEntry(selectedPlayerId); const item = run.inventory.find((candidate) => candidate.kind === "equipment") || global.SEASON1_CONFIG.itemPool.find((candidate) => candidate.kind === "equipment"); if (!entry || !item) throw new Error("Giocatore o equipaggiamento non disponibile"); entry.equippedItem = makeItemInstance(item, "devTools"); }); }
      else if (action === "intensive-training") mutate("intensive-training", {}, () => { const entry = rosterEntry(run.projectSystem?.activePlayerId); if (!entry) throw new Error("Seleziona prima un Progetto"); entry.potentialBoostApplications = [...(entry.potentialBoostApplications || []), { amount: 1, appliedLevel: Number(entry.level || 0) }]; entry.potentialBoost = Number(entry.potentialBoost || 0) + 1; entry.currentOverallBoost = Number(entry.currentOverallBoost || 0) + 1; });
      else if (["add-player", "force-add-player"].includes(action)) { if (!selectedPlayerId) throw new Error("Seleziona un giocatore dai risultati"); const exists = rosterEntry(selectedPlayerId); if (exists) throw new Error("Giocatore già in rosa"); if (run.roster.length >= 15 && action !== "force-add-player") throw new Error("Rosa completa: usa FORZA AGGIUNTA solo per test"); if (action === "force-add-player" && !global.confirm("Forzare l’aggiunta oltre i normali limiti della rosa?")) return; const player = sourcePlayer(selectedPlayerId); mutate(action, { playerId: selectedPlayerId }, () => { run.roster.push({ playerId: String(selectedPlayerId), source: freeAgentsById.has(String(selectedPlayerId)) ? "free_agents" : activeSeason.id, level: run.teamLevel || 0 }); if (run.bench.length < 4) run.bench.push(String(selectedPlayerId)); }); }
