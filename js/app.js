(function (global) {
  ...
  function bindDevelopmentDev() {
    ...
    document.getElementById("dev-project-plus-all")?.addEventListener("click", () => { const state = global.DevelopmentV2.read(); global.DevelopmentV2.PROJECT_RARITIES.forEach((rarity) => state.projects[rarity] += 1); global.DevelopmentV2.write(state); renderDevelopmentCenter("projects"); });
    document.getElementById("dev-project-set")?.addEventListener("click", () => { const state = global.DevelopmentV2.read(); global.DevelopmentV2.PROJECT_RARITIES.forEach((rarity) => { state.projects[rarity] = Math.max(state.projects[rarity], global.DevelopmentV2.COSTS[rarity].projects); }); global.DevelopmentV2.write(state); renderDevelopmentCenter("projects"); });
    document.getElementById("dev-unlock-album")?.addEventListener("click", () => { if (!confirm("Sbloccare tutti i giocatori dell’Album per il test?")) return; Object.values(global.AlbumProgress.ALBUM_COLLECTIONS).forEach((collection) => { const ids = albumCollectionPlayers(collection.id).map((player) => player.playerId); global.AlbumProgress.unlockAlbumPlayers(collection.id, ids, { source: "development-dev" }); }); toast("ALBUM COMPLETO SBLOCCATO — DEV"); renderDevelopmentCenter(); });
