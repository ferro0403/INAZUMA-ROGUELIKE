(function (global) {
    ...
    return `
      <div class="player-detail-layout ${rarityClass(resolved.category)} ${historical ? "player-detail-historical" : ""}">
        ...
        <section class="player-detail-content">
          ...
          <div class="player-detail-tags"><span class="role-chip">${escapeHtml(resolved.position)}</span><span class="role-chip detail-element-chip"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3c4 3 7 6 7 10a7 7 0 0 1-14 0c0-4 3-7 7-10Z"/></svg>${escapeHtml(resolved.element || resolved.type || "-")}</span><span class="role-chip">Lv ${escapeHtml(historical ? global.LevelProgression.formatLevel(player.finalLevel ?? 0, player.seasonId || team?.seasonId || team?.modeId, player.finalLevelUnits) : (player.displayLevelText ?? global.LevelProgression.formatLevel(level, run?.seasonId, player.displayLevelUnits)))}</span></div>
  ...
  function projectCardMarkup(rarity, { selectable = false } = {}) {
    ...
    if (selectable) {
      const buildState = ["Forte", "Elite", "Mondiale", "Leggenda"].includes(rarity) ? `<span>BUILD ${escapeHtml(status.filled)}/${escapeHtml(status.required)}</span>` : "";
  ...
  function showProjectPullInfo(rarity, onClose = null) {
  ...
  function renderProjectPull(pull, onComplete = renderSeasonSelect) {
    if (!pull || pull.claimed) return onComplete();
    const content = `<header><p class="eyebrow">RICOMPENSA RUN</p><h1>SCEGLI UN PROGETTO</h1><p>Una sola scelta. Il Progetto sarà subito disponibile nel Centro di Sviluppo.</p></header><section class="project-pull-grid">${pull.choices.map((r, index) => projectCardMarkup(r, { selectable: true }).replace("SCELTA</span>", `SCELTA 0${index + 1}</span>`)).join("")}</section><p class="project-pull-hint">Tocca una carta per scegliere la ricompensa.</p>`;
    openModal(content, { closeable: false, className: "project-pull-modal" });
    let claiming = false;
    const choices = [...document.querySelectorAll("[data-claim-project]")];
    document.querySelectorAll("[data-project-info]").forEach((button) => button.addEventListener("click", () => showProjectPullInfo(button.dataset.projectInfo, () => renderProjectPull(pull, onComplete))));
    choices.forEach((button) => button.addEventListener("click", () => {
      ...
      if (!result) { claiming = false; choices.forEach((choice) => { choice.disabled = false; }); return; }
      ...
      window.setTimeout(() => { closeModal({ invokeOnClose: false }); onComplete(); }, 180);
    }));
  }
  ...
  function renderDevelopmentCenter(tab = "players") {
    ...
    const buildRarities = ["Forte", "Elite", "Mondiale", "Leggenda"];
    const build = buildRarities.map((rarity) => projectCardMarkup(rarity)).join("");
      ...
      <main class="screen boss-match-screen" data-match-state="${ui.bossMatchState}">
        ...
        <div class="content boss-match-content">
          ...
          <section class="panel boss-match-controls" aria-label="${isSpecial ? "Azioni partita speciale" : "Azioni partita Boss"}">
            <header class="boss-match-actions-heading"><span>Azioni partita</span><i aria-hidden="true"></i></header>
            <div class="boss-match-primary-actions">
              <button type="button" class="btn boss-match-action-cta boss-match-action-cta--primary" id="simulate-boss-match" ${simulating || resolved ? "disabled" : ""}><span class="boss-match-action-icon" aria-hidden="true"><i class="boss-match-play-icon"></i></span><span class="boss-match-action-copy"><strong>${simulating ? "Simulazione..." : "Simula partita"}</strong><small>Avvia la simulazione</small></span><span class="boss-match-action-mark" aria-hidden="true">›</span></button>
              <button type="button" class="btn boss-match-action-cta boss-match-action-cta--secondary" id="edit-boss-team" data-nav="squad" ${resolved ? "disabled" : ""}><span class="boss-match-action-icon" aria-hidden="true"><i class="boss-match-tactics-icon"></i></span><span class="boss-match-action-copy"><strong>Modifica squadra</strong><small>Gestisci titolari</small></span><span class="boss-match-action-mark" aria-hidden="true">›</span></button>
