...
function retryRestore() { const auth = globalThis.InazumaAccount?.getState(); return globalThis.CloudRestoreResumeCoordinator.retry({ auth, readJournal: readRestoreJournal, resumeInterrupted: resumeInterruptedRestore, abandonNonResumable: abandonLegacyRestoreTarget, freshComparison: freshRecoveryComparison, terminalRecoveryActive: () => guard()?.getState?.().status === "terminal-recovery", normalAssociate: () => associateLocalSave({ force: true }), publish: (patch) => publish(patch), onWritable: () => globalThis.PersistenceBootstrapGate?.notify?.() }); }
