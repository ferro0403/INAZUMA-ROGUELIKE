...
async function resolveConflictUseLocal() {
  ...
  } catch (error) { const stale = error?.code === "cloud-cas-conflict"; publish({ status: stale ? "sync-conflict" : "sync-error", error: error?.code || "conflict-resolution-failed", attemptedRevision }, token); } finally { conflictInFlight = null; } })(); return conflictInFlight;
}
