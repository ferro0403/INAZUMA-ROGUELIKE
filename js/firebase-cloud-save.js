...
function readRestoreJournal(uid) { const raw = localStorage.getItem(journalKey(uid)); return raw ? JSON.parse(raw) : null; }
...
async function restoreCloudSave(options = {}) {
  ...
  restoreInFlight = (async () => { let writesStarted = false, restoreStage = "read-manifest-start"; try {
    ...
    publish({ status: "restoring", restoreStage }, token); const applyProvenance = journal.sourceRunProvenance; writesStarted = true;
    applySnapshot(restored, applyProvenance, (stage) => { restoreStage = stage; journal.sourceRunProvenance = core.clone(applyProvenance); journal = writeRestoreJournal(journal, stage); publish({ restoreStage }, token); });
    ...
    publish({ status: clearError ? "restore-repair-needed" : "restored", restoreStage: "complete", revision: manifest.revision, localRevision: manifest.revision, cloudRevision: manifest.revision, hallOfFameCount: hallPayloads.length, lastSyncedAt: metadata.lastSyncedAt, error: clearError, problemSector: null }, token);
  } catch (error) {
