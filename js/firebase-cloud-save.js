...
async function restoreCloudSave(options = {}) {
  ...
  restoreInFlight = (async () => { let writesStarted = false, restoreStage = "read-manifest-start"; try {
    ...
    restoreStage = "apply-local"; publish({ status: "restoring", restoreStage }, token); if (!options.explicitConflict) assertRunProvenanceUnchanged(beforeRunProvenance); const applyProvenance = options.explicitConflict ? captureRunProvenance() : beforeRunProvenance; writesStarted = true; applySnapshot(restored, applyProvenance);
