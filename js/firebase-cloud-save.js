...
function sectorDocument(name, payload, payloadHash, id, timestamp, revision) { return { schemaVersion: 1, revision, sector: name, payload, payloadHash, updatedAt: timestamp, sourceDeviceId: id }; }
function hallDocument(entry, id, timestamp, revision) { return { schemaVersion: 1, revision, ...entry, updatedAt: timestamp, sourceDeviceId: id }; }
...
async function reconcile(uid, id, manifest, prepared, token) {
  ...
  if (!metadata) { publish({ ...base, status: core.inspectLocalProgress(prepared.snapshot).meaningful ? "local-conflict" : "cloud-available" }, token); return; }
...
function onLocalSave(event) { const sector = event?.detail?.sector; if (!core.SECTOR_NAMES.includes(sector) || !cachedManifest || state.uid !== cachedManifest.uid || ["sync-conflict", "local-conflict", "cloud-update-available"].includes(state.status)) return; dirtySectors.add(sector); scheduleSync(); }
...
async function restoreCloudSave() {
  if (restoreInFlight) return restoreInFlight; if (!["cloud-available", "cloud-update-available", "restore-error"].includes(state.status)) return null; const auth = globalThis.InazumaAccount?.getState(), uid = auth?.uid, token = generation, cached = cachedManifest; if (!uid || !cached || cached.uid !== uid || cached.token !== token) return null;
  const id = deviceId(), before = core.readLocalSnapshot(); if (state.status !== "cloud-update-available" && core.inspectLocalProgress(before).meaningful) { publish({ status: "local-conflict" }, token); return null; }
