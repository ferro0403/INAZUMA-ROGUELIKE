...
let automaticUid = null, cachedManifest = null, reloadUsed = false, debounceTimer = null, dirtySectors = new Set(), lastRestoreType = "initial", cloudUpdateBaseline = null;
...
function localSnapshotProof(prepared, cloudRevision = null) { return core.stableSerialize({ cloudRevision, mutationEpoch: guard()?.readEpoch?.() ?? null, sectorHashes: Object.fromEntries(core.SECTOR_NAMES.map((name) => [name, sectorHash(prepared, name)])), hallTeamIds: prepared.hallEntries.map((entry) => entry.hallTeamId).sort(), hallTeamHashes: Object.fromEntries(prepared.hallEntries.map((entry) => [entry.hallTeamId, entry.payloadHash])), runProvenance: prepared.snapshot.runProvenance || null }); }
...
function classifyCloudWriteFailure(error) { return globalThis.InazumaCloudWriteFailurePolicy.classifyCloudWriteFailure(error); }
...
async function reconcile(uid, id, manifest, prepared, token) {
  ...
  if (manifest.revision > metadata.revision) {
    ...
    cloudUpdateBaseline = unchanged ? { uid, revision: manifest.revision, proof: localSnapshotProof(prepared, manifest.revision) } : null;
...
async function syncNow() {
  ...
  } catch (error) { cycle.forEach((name) => dirtySectors.add(name)); console.warn("Cloud autosync:", { code: error?.code || "sync-failed", attemptedRevision, pendingSectors: [...cycle] }); const failure = classifyCloudWriteFailure(error); publish({ ...failure, pendingSectors: [...dirtySectors], attemptedRevision, localRevision: cachedManifest?.data?.revision ?? null }, token); }
  finally { syncInFlight = null; if (dirtySectors.size && !["sync-conflict", "sync-error", "metadata-repair-needed", "local-conflict", "cloud-update-available"].includes(state.status)) scheduleSync(token); } })(); return syncInFlight;
}
function retrySync() { if (!["sync-error", "metadata-repair-needed"].includes(state.status)) return null; return syncNow(); }
function onLocalSave(event) { if (guard()?.isBlocked()) return; const sector = event?.detail?.sector; if (!core.SECTOR_NAMES.includes(sector)) return; if (state.status === "awaiting-local-save") { associateLocalSave({ force: true }); return; } if (!cachedManifest || state.uid !== cachedManifest.uid) return; dirtySectors.add(sector); if (!["sync-conflict", "local-conflict", "cloud-update-available"].includes(state.status)) scheduleSync(); }
...
async function runRestoreRecovery({ uid, token, id, journal, bundle = null }) {
  ...
  if (result.status === "restored") { localStorage.removeItem(`inazuma.cloud.restoreTerminal.${uid}`); globalThis.PersistenceBootstrapGate?.notify?.(); }
  return result;
}
...
async function restoreCloudSave(options = {}) {
  ...
  restoreInFlight = (async () => { try { const bundle = await downloadStableCloudBundle({ uid, token }); if (!bundle.manifest.cloudCommitId) throw restoreError("legacy-cloud-target-not-immutable", "manifest"); const targetSnapshot = core.reconstructSnapshot(bundle.payloads, bundle.hallPayloads), prepared = await core.prepareSnapshot(targetSnapshot), localPrepared = await prepareStableLocalSnapshot(), localSnapshot = localPrepared.snapshot, localMeaningful = core.inspectLocalProgress(localSnapshot).meaningful; const baselineProvesUnchanged = lastRestoreType === "cloud-update" && cloudUpdateBaseline?.uid === uid && cloudUpdateBaseline.revision === bundle.manifest.revision && cloudUpdateBaseline.proof === localSnapshotProof(localPrepared, bundle.manifest.revision); const safeAutomaticReplace = !localMeaningful || baselineProvesUnchanged; if (lastRestoreType !== "explicit-conflict-cloud" && !safeAutomaticReplace && !(await core.compareSnapshots(localSnapshot, targetSnapshot)).equivalent) { publish({ status: "local-conflict", error: "restore-conflict-required" }, token); return { status: "restore-conflict-required" }; } let journal = globalThis.InazumaCloudRestoreProtocol.createJournal({ operationId: globalThis.crypto?.randomUUID?.() || `restore-${Date.now()}`, uid, restoreType: lastRestoreType, safeAutomaticReplace, targetCloudRevision: bundle.manifest.revision, targetCloudCommitId: bundle.manifest.cloudCommitId, targetManifestIdentity: manifestBundleIdentity(bundle.manifest), sourceRunProvenance: captureRunProvenance(), targetRunHashes: Object.fromEntries(globalThis.InazumaCloudRestoreProtocol.RUN_IDS.map((sid) => [sid, prepared.hashes[`run_${sid}`]])), sourceLocalEpoch: guard().readEpoch(), expectedLocalEpoch: guard().readEpoch(), startedAt: new Date().toISOString() }); journal = writeRestoreJournal(journal, "prepared"); cloudUpdateBaseline = null; return await runRestoreRecovery({ uid, token, id, journal, bundle }); } catch (error) { publish({ status: "restore-error", error: error?.code || "restore-failed", problemSector: error?.problemSector || null }, token); throw error; } finally { restoreInFlight = null; } })(); return restoreInFlight;
}
function abandonLegacyRestoreTarget(journal) {
  const auth = globalThis.InazumaAccount?.getState();
  if (!journal || journal.targetCloudCommitId || auth?.status !== "authenticated" || journal.uid !== auth.uid) throw restoreError("restore-ownership-lost");
  const terminalKey = `inazuma.cloud.restoreTerminal.${auth.uid}`;
  const terminal = { schemaVersion: 1, operationId: journal.operationId, stage: "terminal-non-resumable", targetCloudRevision: journal.targetCloudRevision ?? null, reason: "missing-target-cloud-commit-id", recordedAt: new Date().toISOString() };
  localStorage.setItem(terminalKey, JSON.stringify(terminal));
  if (JSON.parse(localStorage.getItem(terminalKey) || "null")?.operationId !== journal.operationId) throw restoreError("restore-journal-cleanup-failed");
  clearRestoreJournal(auth.uid);
  guard().setBlocked({ uid: auth.uid, operationId: null, stage: "fresh-comparison", status: "terminal-recovery", error: terminal.reason });
  globalThis.PersistenceBootstrapGate?.notify?.();
  return terminal;
}
function clearTerminalRecovery(uid) { localStorage.removeItem(`inazuma.cloud.restoreTerminal.${uid}`); guard().clearBlocked(); globalThis.PersistenceBootstrapGate?.notify?.(); }
async function upgradeLegacyCloudBundle(uid, id, token, bundle) {
  ...
  if (!verification.equivalent) throw restoreError("legacy-cloud-upgrade-verification-failed", "manifest");
  publish({ legacyCloudUpgradeStatus: "immutable-copy-published" }, token); return upgraded;
}
async function freshRecoveryComparison() {
  const auth = globalThis.InazumaAccount?.getState(), uid = auth?.uid, token = generation;
  if (auth?.status !== "authenticated" || !uid || guard()?.getState?.().status !== "terminal-recovery") return null;
  const id = deviceId(); publish({ status: "checking", freshComparisonStatus: "canonicalizing-local", pendingSectors: [], error: null }, token);
  try {
    const legacyMigrationResult = [];
    for (const seasonId of globalThis.InazumaCloudRestoreProtocol.RUN_IDS) { const result = globalThis.RunStorage.canonicalizeLegacyPrimary(seasonId, { suppressCloudEvent: true }); if (result?.migrated || result?.repairedHead) legacyMigrationResult.push(result); }
    publish({ freshComparisonStatus: "reading", legacyMigrationResult }, token);
    let bundle = await downloadStableCloudBundle({ uid, token });
    if (!bundle.manifest.cloudCommitId) bundle = await upgradeLegacyCloudBundle(uid, id, token, bundle);
    const localPrepared = await prepareStableLocalSnapshot(), cloudSnapshot = core.reconstructSnapshot(bundle.payloads, bundle.hallPayloads), comparison = await core.compareSnapshots(localPrepared.snapshot, cloudSnapshot);
    cachedManifest = { uid, token, raw: bundle.manifest, data: bundle.manifest };
    if (comparison.equivalent) { clearTerminalRecovery(uid); publish({ status: "synced", freshComparisonStatus: "equivalent", cloudRevision: bundle.manifest.revision, revision: bundle.manifest.revision, pendingSectors: [] }, token); return { classification: "equivalent", manifest: bundle.manifest }; }
    const localMeaningful = core.inspectLocalProgress(localPrepared.snapshot).meaningful, cloudMeaningful = core.inspectLocalProgress(cloudSnapshot).meaningful;
    const classification = !localMeaningful && cloudMeaningful ? "cloud-only" : localMeaningful && !cloudMeaningful ? "local-only" : "conflict";
    publish({ status: classification === "cloud-only" ? "cloud-available" : "local-conflict", freshComparisonStatus: classification, cloudRevision: bundle.manifest.revision, pendingSectors: [] }, token);
    return { classification, manifest: bundle.manifest };
  } catch (error) {
    guard()?.setBlocked?.({ uid, status: "terminal-recovery", stage: "fresh-comparison", error: error?.code || "fresh-comparison-failed" });
    publish({ status: "restore-terminal-error", freshComparisonStatus: "retry-required", error: error?.code || "fresh-comparison-failed", pendingSectors: [] }, token);
    return { classification: "error", status: "restore-terminal-error", retryRequired: true, error: error?.code || "fresh-comparison-failed" };
  }
}
function recoveryRoute(auth) { return globalThis.CloudRestoreResumeCoordinator.route({ auth, readJournal: readRestoreJournal, resumeInterrupted: resumeInterruptedRestore, abandonNonResumable: abandonLegacyRestoreTarget, freshComparison: freshRecoveryComparison, terminalRecoveryActive: () => guard()?.getState?.().status === "terminal-recovery", normalAssociate: () => auth?.profileComplete ? associateLocalSave() : null, publish: (patch) => publish(patch), onWritable: () => globalThis.PersistenceBootstrapGate?.notify?.() }); }
function retryRestore() { const auth = globalThis.InazumaAccount?.getState(); if (!readRestoreJournal(auth?.uid) && guard()?.getState?.().status === "terminal-recovery") return freshRecoveryComparison(); return globalThis.CloudRestoreResumeCoordinator.retry({ auth, readJournal: readRestoreJournal, resumeInterrupted: resumeInterruptedRestore, abandonNonResumable: abandonLegacyRestoreTarget, freshComparison: freshRecoveryComparison, normalAssociate: () => associateLocalSave({ force: true }), publish: (patch) => publish(patch), onWritable: () => globalThis.PersistenceBootstrapGate?.notify?.() }); }
...
async function resolveConflictUseLocal() {
  ...
  conflictInFlight = (async () => { let attemptedRevision = null; try {
    ...
    const db = globalThis.InazumaAccount.getFirestoreInstance(), commitId = newCloudCommitId();
    const committed = await commitBundle(uid, id, prepared, db, old, commitId);
    if (!current(token, uid)) return; cached.raw = committed; cached.data = core.validateManifest(committed, uid); const metadata = metadataFrom(uid, id, cached.data, prepared, readMetadata(uid, id) || {}); try { writeMetadata(uid, metadata); } catch (error) { throw Object.assign(error, { code: "metadata-repair-needed", serverCommitted: true }); } dirtySectors.clear(); if (guard()?.getState?.().status === "terminal-recovery") clearTerminalRecovery(uid); publish({ status: "synced", revision: nextRevision, localRevision: nextRevision, cloudRevision: nextRevision, pendingSectors: [], lastSyncedAt: metadata.lastSyncedAt, error: null }, token);
  } catch (error) { const failure = classifyCloudWriteFailure(error); publish({ ...failure, attemptedRevision, pendingSectors: [...dirtySectors] }, token); } finally { conflictInFlight = null; } })(); return conflictInFlight;
}
...
function authChanged(event) {
  ...
  generation += 1; clearTimer(); associationInFlight = null; syncInFlight = null; checkInFlight = null; conflictInFlight = null; cachedManifest = null; cloudUpdateBaseline = null; dirtySectors = new Set(); reloadUsed = false;
...
globalThis.addEventListener("inazuma:auth-state-changed", authChanged); globalThis.addEventListener("inazuma:local-save-committed", onLocalSave); globalThis.addEventListener("online", () => { if (["sync-error", "metadata-repair-needed"].includes(state.status)) scheduleSync(); }, { passive: true });
const ready = Promise.resolve(globalThis.InazumaAccount?.ready).then(() => authChanged());
globalThis.InazumaCloudSave = Object.freeze({ ready, getState: () => ({ ...state, pendingSectors: [...dirtySectors] }), classifyCloudWriteFailure, associateLocalSave, resumeInterruptedRestore, freshRecoveryComparison, upgradeLegacyCloudBundle, abandonNonResumableRestore: abandonLegacyRestoreTarget, retryAssociation: () => { automaticUid = null; return associateLocalSave({ force: true }); }, syncNow, retrySync, checkForCloudUpdate, updateFromCloud, restoreCloudSave, retryRestore, requestConflictResolution, resolveConflictUseLocal, resolveConflictUseCloud, reloadAfterRestore });
