...
function authChanged(event) { const auth = event?.detail || globalThis.InazumaAccount?.getState(); const sameUid = auth?.status === "authenticated" && auth.uid && state.uid === auth.uid; if (sameUid) { if (auth.profileComplete && !associationInFlight && !cachedManifest && automaticUid !== auth.uid && state.status !== "awaiting-local-save") associateLocalSave(); return; } generation += 1; clearTimer(); associationInFlight = null; syncInFlight = null; checkInFlight = null; cachedManifest = null; dirtySectors = new Set(); reloadUsed = false; if (auth?.status === "authenticated" && auth.uid) { publish({ ...initial, uid: auth.uid }); if (auth.profileComplete) associateLocalSave(); } else if (auth?.status === "signed-out") { automaticUid = null; publish({ ...initial, status: "signed-out" }); } else publish({ ...initial, uid: auth?.uid || null }); }
