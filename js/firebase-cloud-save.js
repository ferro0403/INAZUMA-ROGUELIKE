...
async function restoreCloudSave(options = {}) {
  ...
  restoreInFlight = (async () => { try { const bundle = await downloadStableCloudBundle({ uid, token }); if (!bundle.manifest.cloudCommitId) throw restoreError("legacy-cloud-target-not-immutable", "manifest"); const prepared = await core.prepareSnapshot(core.reconstructSnapshot(bundle.payloads, bundle.hallPayloads)); let journal = globalThis.InazumaCloudRestoreProtocol.createJournal({ operationId: globalThis.crypto?.randomUUID?.() || `restore-${Date.now()}`, uid, restoreType: lastRestoreType, targetCloudRevision: bundle.manifest.revision, targetCloudCommitId: bundle.manifest.cloudCommitId, targetManifestIdentity: manifestBundleIdentity(bundle.manifest), sourceRunProvenance: captureRunProvenance(), targetRunHashes: Object.fromEntries(globalThis.InazumaCloudRestoreProtocol.RUN_IDS.map((sid) => [sid, prepared.hashes[`run_${sid}`]])), sourceLocalEpoch: guard().readEpoch(), expectedLocalEpoch: guard().readEpoch(), startedAt: new Date().toISOString() }); journal = writeRestoreJournal(journal, "prepared"); return await runRestoreRecovery({ uid, token, id, journal, bundle }); } catch (error) { publish({ status: "restore-error", error: error?.code || "restore-failed", problemSector: error?.problemSector || null }, token); throw error; } finally { restoreInFlight = null; } })(); return restoreInFlight;
}
