...
`playerCard()` remains in `app.js` as a **SHARED RUNTIME ADAPTER / FUTURE CLEANUP CANDIDATE** because it combines run/progression resolution with large-card presentation. `playerTeamIdentity`, `historicalTeamIdentity`, and `teamLogoMarkup` also remain runtime-owned adapters and are injected into `PlayerView`; moving them would incorrectly transfer roster/season-team authority into presentation.

The production test harness intentionally retains a no-op `MutationObserver`. The legacy badge scripts are loaded and their required DOM contract is asserted, but asynchronous badge augmentation is explicitly recorded as **LEGACY AUGMENTATION NOT FULLY EMULATED BY TEST HARNESS** rather than represented as browser-level E2E coverage.
